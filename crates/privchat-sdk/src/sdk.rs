...
use crate::network::{NetworkMonitor, NetworkStatus, NetworkStatusListener, NetworkStatusEvent};
...
use crate::storage::queue::{SendQueueManager, SendTask, MessageData, QueuePriority};
use crate::connection_state::{ConnectionStateManager, ConnectionProtocol};
...
impl NetworkStatusListener for DefaultNetworkStatusListener {
    ...
    async fn start_monitoring(&self) -> crate::error::Result<broadcast::Receiver<NetworkStatusEvent>> {
...
pub struct PrivchatSDK {
    ...
    /// 发送消息消费者
    send_consumer: Arc<RwLock<Option<Arc<crate::storage::queue::send_consumer::SendConsumerRunner>>>>,
...
impl PrivchatSDK {
    ...
    pub async fn initialize(config: PrivchatConfig) -> Result<Arc<Self>> {
        ...
        if let Some(offset_seconds) = config.timezone_offset_seconds {
            ...
            info!("已设置时区偏移: {} 秒 ({} 小时)", offset_seconds, offset_seconds / 3600);
        } else {
        ...
        // === 第6层：在线状态管理器 ===
        let presence_manager = Arc::new(crate::presence::PresenceManager::new(event_manager.clone()));
        info!("在线状态管理器初始化完成");
        ...
        // === 第9层：生命周期管理器 ===
        let lifecycle_manager = Arc::new(tokio::sync::RwLock::new(crate::lifecycle::LifecycleManager::new()));
        info!("生命周期管理器初始化完成");
        ...
        let message_rate_limiter = Arc::new(MessageRateLimiter::new(
            MessageRateLimiterConfig::default()
        ));
        
        let rpc_rate_limiter = RpcRateLimiter::new(
            RpcRateLimiterConfig::default()
        );
        ...
        info!("✅ Snowflake ID 生成器初始化完成 (machine_id={}, data_center_id={})", machine_id, data_center_id);
        
        // === 第11层：HTTP 客户端（文件上传/下载）===
        let http_client = if let Some(ref file_api_url) = config.file_api_base_url {
            match crate::http_client::FileHttpClient::new(&config.http_client_config, Some(file_api_url.clone())) {
                Ok(client) => {
    ...
    pub async fn register(
        ...
        let response: serde_json::Value = client.call_rpc_typed(
        ...
        let user_id = response["user_id"].as_u64()
        ...
        info!("✅ 用户注册成功: username={}, user_id={}", username, user_id);
    ...
    pub async fn login(
        ...
        info!("✅ 用户登录成功: username={}, user_id={}", username, user_id);
    ...
    pub async fn authenticate(
        ...
        self.connection_state.set_user_info(
        ...
        if let Some(server_info) = &session.server_info {
            self.connection_state.set_server_metadata(
        ...
        // 更新网络状态为 Online
        self.network.set_status(crate::network::NetworkStatus::Online).await;
        ...
        *self.transport_disconnect_rx.write().await = Some(disconnect_rx);
        ...
        tokio::spawn(async move {
            ...
            while let Some(first_push) = message_rx.recv().await {
                ...
                let mut channel_timeline: HashMap<(u64, i32), Vec<crate::events::TimelineMessage>> = HashMap::new();

                for push_msg in batch {
                    debug!("收到推送消息: message_id={}, from={}", push_msg.server_message_id, push_msg.from_uid);
                    ...
                    let content = if let Ok(payload_json) = serde_json::from_slice::<serde_json::Value>(&push_msg.payload) {
                    ...
                    let message = Message {
                        ...
                        expire_time: if push_msg.expire > 0 { Some(push_msg.expire as i64) } else { None },
                    ...
                    let row_id = match storage.save_received_message(&message).await {
                        Ok(id) => id,
                        Err(e) => {
                            warn!("保存接收消息到数据库失败: {}", e);
                            0
                    ...
                    if row_id > 0 {
                        debug!("✅ 消息已保存到数据库: message_id={}, row_id={}", push_msg.server_message_id, row_id);
                        ...
                        if let Some(uid) = storage.get_current_user_id().await {
                            ...
                            tokio::spawn(async move {
                                if let Err(e) = Self::download_thumbnail_after_receive(
                                    ...
                                    warn!("下载消息缩略图失败: row_id={}, error={:?}", msg_row_id, e);
                                }
                            });
                        }
                    }
                    ...
                    match pts_manager.has_gap(channel_id, channel_type, server_pts as u64).await {
                        Ok(true) => {
                            warn!("检测到 pts 间隙: channel_id={}, channel_type={}", channel_id, channel_type);
                            if let Some(sync_engine) = sync_engine_ref.read().await.as_ref() {
                                ...
                                tokio::spawn(async move {
                                    ...
                                    match sync_engine_clone.sync_channel(channel_id, channel_type).await {
                                        Ok(state) => info!("补齐同步完成: channel_id={}, state={:?}", channel_id, state.state),
                                        Err(e) => error!("补齐同步失败: channel_id={}, error={:?}", channel_id, e),
                                    }
                                });
                            }
                        }
                        Ok(false) => {
                            if let Err(e) = pts_manager.update_local_pts(channel_id, channel_type, server_pts as u64).await {
                                warn!("更新本地 pts 失败: {:?}", e);
                            }
                        }
                        Err(e) => warn!("间隙检测失败: {:?}", e),
                    ...
                    let timestamp_ms_u64: u64 = {
                        let ts = push_msg.timestamp as u64;
                        if ts < 1_000_000_000_000u64 { ts * 1000 } else { ts }
                    };
                    ...
                    // 2. 获取频道信息（名称、头像等）
                    let channel = match storage_clone.get_channel(channel_id, channel_type).await {
                        Ok(ch) => ch,
                        Err(e) => {
                            warn!("获取频道信息失败: {:?}", e);
                            None
                    ...
                    let member_count = if channel_type == 2 {
                        match storage_clone.get_group_members(channel_id, None, None).await {
                    ...
                    if let Some(conv) = conv {
                        ...
                        let conv_event = SDKEvent::ChannelListUpdate {
                            update_kind: ChannelListUpdateKind::Update {
                                channel: entry,
                            },
                            timestamp,
                        };
        ...
        self.connection_state.set_server_info(
    ...
    async fn get_queue_manager(&self) -> Result<Arc<SendQueueManager>> {
        let uid = self.storage().get_current_user_id().await
    ...
    async fn start_send_consumer(&self) -> Result<()> {
        ...
        use crate::storage::queue::send_consumer::{SendConsumerRunner, SendConsumerConfig};
    ...
    async fn start_file_send_consumer(&self) -> Result<()> {
        ...
        use crate::storage::queue::{FileConsumerRunner, FileConsumerConfig};
    ...
    async fn cleanup_tmp_files(&self) -> Result<()> {
        ...
        while let Some(entry) = entries.next_entry().await
            ...
            while let Some(t) = tmp_entries.next_entry().await
        ...
        if let Some(mut rx) = rx {
            tokio::spawn(async move {
                while rx.recv().await.is_some() {
                    ...
                    self.event_manager.emit(crate::events::event_builders::connection_state_changed(
                        EventConnectionState::Disconnected,
                        EventConnectionState::Reconnecting,
                    )).await;
                    info!("开始自动重连（首次 {}s 后尝试）", Self::AUTO_RECONNECT_FIRST_DELAY_SECS);

                    tokio::time::sleep(Duration::from_secs(Self::AUTO_RECONNECT_FIRST_DELAY_SECS)).await;
    ...
    pub async fn send_message(&self, channel_id: u64, content: &str) -> Result<u64> {
        self.send_message_with_options(channel_id, content, crate::events::SendMessageOptions::default()).await
    }
    ...
    pub async fn send_message_with_options(
        ...
        info!("🔍 [DEBUG] send_message 开始: channel_id={}, content={}", channel_id, content);
        ...
        info!("准备发送消息: channel_id={}, content={}", channel_id, content);
        ...
        let reply_to_server_id = if let Some(reply_id) = options.in_reply_to_message_id {
            self.storage.get_message_by_id(reply_id as i64).await.ok().flatten().and_then(|m| m.server_message_id).unwrap_or(0)
        } else {
            0u64
        };
        ...
        let row_id = self.storage.save_received_message(&message).await?;
        info!("✅ 消息已保存到数据库: row_id={}, status=pending", row_id);
        ...
        if reply_to_server_id != 0 {
            message_data = message_data.with_extra("reply_to_message_id".to_string(), reply_to_server_id.to_string());
        }
        if !options.mentions.is_empty() {
            message_data = message_data.with_extra("mentioned_user_ids".to_string(), serde_json::to_string(&options.mentions).unwrap_or_default());
        }
        if options.silent {
        ...
        if let Some(client_extra) = &options.extra {
            if let Some(obj) = client_extra.as_object() {
                for (key, value) in obj {
                    let value_str = if value.is_string() { value.as_str().unwrap_or("").to_string() } else { value.to_string() };
        ...
        self.connection_state.increment_sent(content.len() as u64).await;
    ...
    pub async fn send_attachment_from_path(
        ...
        let user_id = self.user_id().await.ok_or_else(|| PrivchatSDKError::NotConnected)?;
        ...
        let file_size = tokio::fs::metadata(file_path).await.map(|m| m.len()).unwrap_or(0) as i64;
        ...
        let pre_uploaded_thumbnail_file_id = if message_type_str == "video" && video_hook.is_none() {
            let thumb_token_req = privchat_protocol::rpc::file::upload::FileRequestUploadTokenRequest {
                user_id,
            ...
            let thumb_token = {
                ...
                let c = guard.as_mut().ok_or_else(|| PrivchatSDKError::NotConnected)?;
            ...
            let thumb_resp = http.upload_file_bytes(
        ...
        info!("✅ 附件已入文件队列: message_id={}, local_message_id={}", message_id, local_message_id);
    ...
    pub async fn send_attachment_bytes(
        ...
        info!("📤 开始发送附件（从内存）: {} ({} bytes)", filename, data.len());
        ...
        let upload_token_req = privchat_protocol::rpc::file::upload::FileRequestUploadTokenRequest {
            user_id,
        ...
        let upload_token_resp = {
            ...
            let client_ref = client.as_mut()
                .ok_or_else(|| PrivchatSDKError::NotConnected)?;
            client_ref.file_request_upload_token(upload_token_req).await?
        };
        
        info!("✅ 获取上传 token 成功: upload_url={}", upload_token_resp.upload_url);
        
        // 3. 上传文件（HTTP）
        let upload_resp = {
            let http_client_guard = self.http_client.read().await;
            let http_client = http_client_guard.as_ref()
                .ok_or_else(|| PrivchatSDKError::Other("HTTP 客户端未初始化，请配置 file_api_base_url".to_string()))?;
            
            http_client.upload_file_bytes(
        ...
        info!("✅ 文件上传成功: file_id={}, file_url={}", upload_resp.file_id, upload_resp.file_url);
        ...
        // 5. 发送消息
        let local_message_id = self.send_message_with_options(
            channel_id,
            &content,
            options,
        ).await?;
        ...
        info!("✅ 附件消息发送成功: local_message_id={}, file_id={}", local_message_id, file_id);
    ...
    pub async fn download_attachment_to_cache(
        ...
        info!("📥 开始下载附件到缓存: file_id={}, file_url={}", file_id, file_url);
        
        self.check_initialized().await?;
        
        // 1. 检查文件是否已在缓存中
        let media_index = self.storage.media_index()
        ...
        http_client.download_file(file_url, &temp_file, progress.clone()).await?;
        drop(http_client_guard);
        
        // 5. 将文件添加到 MediaIndex（会自动移动到正确的目录）
        let _file_record = media_index.add_file(&temp_file, Some(file_id.to_string())).await?;
        
        // 6. 获取最终缓存路径
        let cache_path = media_index.get_file_path(file_id).await?
            .ok_or_else(|| PrivchatSDKError::Other("文件下载后无法获取缓存路径".to_string()))?;
        
        info!("✅ 附件下载到缓存成功: file_id={}, cache_path={}", file_id, cache_path.display());
    ...
    pub async fn download_attachment_to_path(
        ...
        info!("📥 开始下载附件到指定路径: file_url={}, output_path={}", file_url, output_path.display());
        ...
        http_client.download_file(file_url, output_path, progress).await?;
        drop(http_client_guard);
    ...
    pub async fn download_attachment_to_message_dir(
        ...
        self.download_attachment_to_path(file_url, &output_path, progress).await?;
        Ok(output_path)
    }
    ...
    pub async fn send_message_with_input(&self, input: &MessageInput) -> Result<u64> {
        ...
        let message_id = self.snowflake.next_id()
    ...
    pub async fn mark_as_read(&self, channel_id: u64, message_id: u64) -> Result<()> {
        ...
        info!("标记消息为已读: channel_id={}, message_id(id)={}", channel_id, message_id);
    ...
    /// # 返回
    /// 根据 message.id 获取消息
    pub async fn get_message_by_id(&self, id: u64) -> Result<Option<crate::storage::entities::Message>> {
        self.check_initialized().await?;
    ...
    pub async fn recall_message(&self, id: u64) -> Result<()> {
        self.check_initialized().await?;
        let msg = self.storage.get_message_by_id(id as i64).await?
    ...
    pub async fn edit_message(&self, id: u64, new_content: &str) -> Result<()> {
        self.check_initialized().await?;
        self.storage.update_message_content(id as i64, new_content).await?;
        info!("✅ 消息已编辑: id={}", id);
        Ok(())
    }
    ...
    pub async fn add_reaction(&self, message_id: u64, emoji: &str) -> Result<()> {
        ...
        let msg = self.storage.get_message_by_id(message_id as i64).await?
            .ok_or_else(|| PrivchatSDKError::Other(format!("消息不存在: id={}", message_id)))?;
        let server_message_id = msg.server_message_id
            .ok_or_else(|| PrivchatSDKError::Other("消息尚未同步到服务端，无法添加反应".to_string()))?;
        
        info!("添加表情反馈: id={}, server_msg_id={}, emoji={}", message_id, server_message_id, emoji);
        ...
        self.storage.add_message_reaction(message_id as i64, user_id, emoji).await?;
    ...
    pub async fn remove_reaction(&self, message_id: u64, emoji: &str) -> Result<()> {
        ...
        let msg = self.storage.get_message_by_id(message_id as i64).await?
            .ok_or_else(|| PrivchatSDKError::Other(format!("消息不存在: id={}", message_id)))?;
        let server_message_id = msg.server_message_id
            .ok_or_else(|| PrivchatSDKError::Other("消息尚未同步到服务端，无法移除反应".to_string()))?;
        
        info!("移除表情反馈: id={}, server_msg_id={}, emoji={}", message_id, server_message_id, emoji);
    ...
    /// }
    /// ```
    pub async fn reactions(&self, _channel_id: u64, message_id: u64) -> Result<Vec<crate::events::ReactionChip>> {
        self.check_initialized().await?;
        self.check_connected().await?;
        ...
        let msg = self.storage.get_message_by_id(message_id as i64).await?
            .ok_or_else(|| PrivchatSDKError::Other(format!("消息不存在: id={}", message_id)))?;
        let server_message_id = msg.server_message_id
            .ok_or_else(|| PrivchatSDKError::Other("消息尚未同步到服务端，无法获取反应列表".to_string()))?;
        ...
        info!("✅ 获取消息反应列表成功: id={}, 反应数={}", message_id, chips.len());
    ...
    /// }
    /// ```
    pub async fn is_event_read_by(&self, _channel_id: u64, message_id: u64, user_id: u64) -> Result<bool> {
        self.check_initialized().await?;
        ...
        let msg = self.storage.get_message_by_id(message_id as i64).await?
            .ok_or_else(|| PrivchatSDKError::Other(format!("消息不存在: id={}", message_id)))?;
        let server_message_id = msg.server_message_id
            .ok_or_else(|| PrivchatSDKError::Other(format!("消息尚未同步到服务器: id={}", message_id)))?;
    ...
    /// # 返回
    /// - 返回初始的在线状态信息
    pub async fn subscribe_presence(&self, user_ids: Vec<u64>) -> Result<HashMap<u64, privchat_protocol::presence::OnlineStatusInfo>> {
        self.check_initialized().await?;
        self.check_connected().await?;
        ...
        let response: SubscribePresenceResponse = client.subscribe_presence(subscribe_request).await?;
        
        // 2. 更新本地订阅状态
        self.presence_manager.add_subscription(user_ids.clone()).await;
        
        // 3. 更新本地缓存
        self.presence_manager.batch_update_status(response.initial_statuses.clone()).await;
    ...
    pub async fn unsubscribe_presence(&self, user_ids: Vec<u64>) -> Result<()> {
        ...
        // 2. 更新本地订阅状态
        self.presence_manager.remove_subscription(user_ids.clone()).await;
    ...
    pub async fn get_presence(&self, user_id: u64) -> Option<privchat_protocol::presence::OnlineStatusInfo> {
    ...
    pub async fn batch_get_presence(&self, user_ids: &[u64]) -> HashMap<u64, privchat_protocol::presence::OnlineStatusInfo> {
    ...
    /// # 返回
    /// - 返回最新的在线状态信息
    pub async fn fetch_presence(&self, user_ids: Vec<u64>) -> Result<HashMap<u64, privchat_protocol::presence::OnlineStatusInfo>> {
        self.check_initialized().await?;
        self.check_connected().await?;
        ...
        // 2. 更新本地缓存
        self.presence_manager.batch_update_status(response.statuses.clone()).await;
    ...
    pub async fn send_typing(
        ...
        // 检查是否需要发送（防抖）
        let should_send = self.typing_manager
        ...
        let mut client_guard = self.client.write().await;
        let _client = client_guard.as_mut()
            .ok_or(PrivchatSDKError::NotConnected)?;
                    ...
                    Ok(event) => {
                        match event {
                            SDKEvent::TypingStarted(typing_event) => {
                                callback(typing_event.user_id.to_string(), typing_event.channel_id.to_string(), true);
                            }
                            SDKEvent::TypingStopped(typing_event) => {
                                callback(typing_event.user_id.to_string(), typing_event.channel_id.to_string(), false);
                            }
                            SDKEvent::TypingIndicator(typing_event) => {
                                callback(typing_event.user_id.to_string(), typing_event.channel_id.to_string(), typing_event.is_typing);
                            }
                            _ => {}
                        }
                    }
    ...
    pub async fn search_users(&self, query: &str) -> Result<serde_json::Value> {
        ...
        let response: serde_json::Value = client.call_rpc_typed(routes::account_search::QUERY, request).await?;
    ...
    pub async fn send_friend_request(&self, to_user_id: u64, remark: Option<&str>, search_session_id: Option<String>) -> Result<serde_json::Value> {
        use privchat_protocol::rpc::FriendApplyRequest;
        
        self.check_initialized().await?;
        self.check_connected().await?;
        
        let from_user_id = self.user_id().await
            .ok_or(PrivchatSDKError::NotConnected)?;
        ...
        let response = client.call_rpc_typed(routes::friend::APPLY, request).await?;
    ...
    pub async fn get_friend_pending_requests(&self) -> Result<privchat_protocol::rpc::contact::friend::FriendPendingResponse> {
        use privchat_protocol::rpc::contact::friend::{FriendPendingRequest, FriendPendingResponse};
    ...
    pub async fn accept_friend_request(&self, from_user_id: u64) -> Result<serde_json::Value> {
        ...
        let target_user_id = self.user_id().await
            .ok_or(PrivchatSDKError::NotConnected)?;
        ...
        let channel_id: u64 = client.call_rpc_typed(routes::friend::ACCEPT, request).await?;
        
        // ✅ 保存 channel 到数据库（若已存在相同 channel_id 的私聊会话则跳过，避免列表两条）
        if channel_id > 0 {
            if self.storage.get_direct_channel_by_id(channel_id).await?.is_some() {
                debug!("私聊会话已存在，跳过保存: channel_id={}", channel_id);
            } else {
                info!("💾 保存私聊会话到数据库: channel_id={}, target_user={}", channel_id, from_user_id);
        ...
        info!("✅ 好友请求已接受: from_user={}, channel_id={}", from_user_id, channel_id);
    ...
    pub async fn reject_friend_request(&self, from_user_id: u64) -> Result<serde_json::Value> {
        ...
        let _: bool = client.call_rpc_typed(routes::friend::REJECT, request).await?;
    ...
    pub async fn get_or_create_direct_channel(
        ...
        use privchat_protocol::rpc::channel::{GetOrCreateDirectChannelRequest, GetOrCreateDirectChannelResponse};
        ...
        if user_id == target_user_id {
            return Err(PrivchatSDKError::Other("不能与自己创建私聊会话".to_string()));
        }
        ...
        let response: GetOrCreateDirectChannelResponse =
            client.call_rpc_typed(routes::channel::DIRECT_GET_OR_CREATE, request).await?;
        ...
        if channel_id > 0 {
            ...
            if self.storage.get_direct_channel_by_id(channel_id).await?.is_some() {
                ...
                info!("💾 保存私聊会话到数据库: channel_id={}, target_user={}, created={}", channel_id, target_user_id, created);
    ...
    pub async fn get_friends(
        ...
        limit: u32,
        offset: u32,
    ) -> Result<Vec<(crate::storage::entities::Friend, crate::storage::entities::User)>> {
        self.check_initialized().await?;
    ...
    pub async fn sync_entities(
        ...
        info!("🔄 开始同步实体（ENTITY_SYNC_V1）: {} scope={:?}", entity_type.as_str(), scope);
    ...
    pub async fn delete_friend(&self, friend_user_id: u64) -> Result<serde_json::Value> {
        ...
        let _: bool = client.call_rpc_typed(routes::friend::DELETE, request).await?;
    ...
    /// - `name`: 群组名称
    /// - `member_ids`: 初始成员ID列表
    pub async fn create_group(&self, name: &str, member_ids: Vec<u64>) -> Result<privchat_protocol::rpc::group::group::GroupCreateResponse> {
        ...
        let request = GroupCreateRequest {
            ...
            name: name.to_string(),
            description: None,
            member_ids: if member_ids.is_empty() { None } else { Some(member_ids.clone()) },
        };
        
        use privchat_protocol::rpc::group::group::GroupCreateResponse;
        let response: GroupCreateResponse = client.call_rpc_typed(routes::group::CREATE, request).await?;
        
        info!("✅ 群组已创建: name={}, members={}, group_id={}", name, member_ids.len(), response.group_id);
        Ok(response)
    }
    
    /// 邀请成员加入群组
    pub async fn invite_to_group(&self, group_id: u64, user_ids: Vec<u64>) -> Result<serde_json::Value> {
        use privchat_protocol::rpc::GroupMemberAddRequest;
        
        self.check_initialized().await?;
        self.check_connected().await?;
        
        let inviter_id = self.user_id().await
            .ok_or(PrivchatSDKError::NotConnected)?;
        ...
        for user_id in &user_ids {
            ...
            match client.call_rpc_typed::<_, serde_json::Value>(routes::group_member::ADD, request).await {
                Ok(_) => success_count += 1,
                Err(e) => warn!("⚠️ 添加成员 {} 失败: {}", user_id, e),
            }
        }
        
        info!("✅ 成员已邀请加入群组: group={}, 成功={}/{}", group_id, success_count, user_ids.len());
        Ok(serde_json::json!({
    ...
    pub async fn join_group_by_qrcode(
        ...
        let response: privchat_protocol::rpc::GroupQRCodeJoinResponse = 
            client.call_rpc_typed(routes::group_qrcode::JOIN, request).await?;
        
        info!("✅ 已加入群组: group_id={}, status={}", response.group_id, response.status);
    ...
    pub async fn leave_group(&self, group_id: u64) -> Result<bool> {
        ...
        let _: bool = client.call_rpc_typed(routes::group_member::LEAVE, request).await?;
    ...
    pub async fn get_group_members(
        ...
        self.check_initialized().await?;
        self.storage().get_group_members(group_id, limit, offset).await
    }
    ...
    /// - `limit`: 每页数量
    /// - `offset`: 偏移量
    pub async fn get_groups(&self, limit: u32, offset: u32) -> Result<Vec<crate::storage::entities::Group>> {
        self.check_initialized().await?;
    ...
    pub async fn get_all_user_settings(&self) -> Result<std::collections::HashMap<String, serde_json::Value>> {
        self.check_initialized().await?;
    ...
    pub async fn remove_group_member(&self, group_id: u64, user_id: u64) -> Result<bool> {
        ...
        let operator_id = self.user_id().await
            .ok_or(PrivchatSDKError::NotConnected)?;
        ...
        let _: bool = client.call_rpc_typed(routes::group_member::REMOVE, request).await?;
    ...
    async fn get_messages_before(
        ...
        debug!("[Rust SDK] 📖 准备从本地读: channel_id={}, before_id={}, limit={}", channel_id, before_id, limit);
        let messages = self.storage().get_messages_before(channel_id, before_id, limit).await?;
        debug!("[Rust SDK] 📖 本地读结果: channel_id={}, 返回 {} 条", channel_id, messages.len());
        debug!("✅ [Local] 查询消息成功: channel_id={}, before_id={}, count={}", 
               channel_id, before_id, messages.len());
    ...
    pub async fn paginate_back(
        ...
        let messages = self.storage().get_messages_before(channel_id, before_message_id, count).await?;
        
        debug!("✅ [Paginate Back] 加载消息成功: channel_id={}, before_id={}, count={}", 
               channel_id, before_message_id, messages.len());
        ...
        if !messages.is_empty() {
            ...
            let timeline_messages: Vec<TimelineMessage> = messages.iter()
                .map(|msg| {
            ...
            if !timeline_messages.is_empty() {
                ...
                self.event_manager.emit(SDKEvent::TimelineDiff {
                    channel_id,
    ...
    pub async fn paginate_forward(
        ...
        let messages = self.storage().get_messages_after(channel_id, after_message_id, count).await?;
        
        debug!("✅ [Paginate Forward] 加载消息成功: channel_id={}, after_id={}, count={}", 
               channel_id, after_message_id, messages.len());
    ...
    pub async fn sync_messages(&self, channel_id: u64, limit: Option<u32>) -> Result<usize> {
        ...
        info!("🔄 开始同步频道 {} 的消息历史（最多 {} 条）...", channel_id, sync_limit);
        ...
        debug!("[Rust SDK] 🔄 准备调用 RPC: route=message_history/get, params={:?}", request);
        let response: serde_json::Value = client.call_rpc_typed(routes::message_history::GET, request).await?;
        debug!("[Rust SDK] 📥 收到服务器响应: {}", serde_json::to_string_pretty(&response).unwrap_or_else(|_| "无法序列化".to_string()));
                    ...
                    Ok(message) => {
                        ...
                        match self.storage().save_message(&message).await {
                                    ...
                                    if let Some(uid) = self.storage().get_current_user_id().await {
                                        ...
                                        tokio::spawn(async move {
                                            if let Err(e) = Self::download_thumbnail_after_receive(
                                                ...
                                                warn!("同步消息缩略图下载失败: row_id={}, error={:?}", row_id, e);
                                            }
                                        });
                                    }
                                }
                            ...
                            Err(e) => {
                                ...
                                warn!("保存消息失败: message_id={:?}, error={}", message.server_message_id, e);
                            }
                        }
                    }
                    Err(e) => {
                        debug!("[Rust SDK] ❌ 解析消息失败: channel_id={}, error={:?}", channel_id, e);
        ...
        debug!("[Rust SDK] ✅ 消息历史同步完成：频道 {}, 本次新入库 {} 条", channel_id, synced_count);
    ...
    fn parse_message_from_json(&self, value: &serde_json::Value, channel_id: u64) -> Result<crate::storage::entities::Message> {
        let now = chrono::Utc::now().timestamp_millis();
        
        // 支持 from_uid 或 sender_id
        let from_uid = value.get("from_uid")
            .and_then(|v| v.as_u64())
        ...
        let message_type = value.get("metadata")
        ...
        Ok(crate::storage::entities::Message {
            ...
            channel_id,
            channel_type: value.get("channel_type").and_then(|v| v.as_i64()).unwrap_or(0) as i32,
            timestamp,
            from_uid,
            message_type,
            ...
            voice_status: value.get("voice_status").and_then(|v| v.as_i64()).unwrap_or(0) as i32,
            created_at: value.get("created_at").and_then(|v| v.as_i64()).unwrap_or(now),
            updated_at: value.get("updated_at").and_then(|v| v.as_i64()).unwrap_or(now),
            searchable_word: value.get("searchable_word")
                .and_then(|v| v.as_str())
                .unwrap_or("")
                .to_string(),
            // 协议字段，仅存库；所有操作只用 message.id，无值时写 0
            local_message_id: 0,
            is_deleted: value.get("is_deleted").and_then(|v| v.as_i64()).unwrap_or(0) as i32,
            ...
            flame_second: value.get("flame_second").and_then(|v| v.as_i64()).unwrap_or(0) as i32,
            ...
            revoked_at: value.get("revoked_at").and_then(|v| v.as_i64()).unwrap_or(0),
    ...
    pub async fn search_channel(
        ...
        // 从本地数据库搜索
        let messages = self.storage.search_messages(channel_id, 1, query, Some(limit)).await?;
        
        // 转换为 SearchHit
        let hits: Vec<crate::events::SearchHit> = messages.iter()
            .filter_map(|msg| {
                msg.server_message_id.map(|msg_id| crate::events::SearchHit {
                    channel_id: msg.channel_id,
    ...
    pub async fn search_messages(&self, query: &str, channel_id: Option<&str>) -> Result<serde_json::Value> {
        self.check_initialized().await?;
        let channel_type = 1i32;
        let limit = 100u32;
        let channel_id_u64 = channel_id.and_then(|s| s.parse::<u64>().ok());
        let messages = match channel_id_u64 {
            Some(cid) => self.storage.search_messages(cid, channel_type, query, Some(limit)).await?,
            None => vec![],
        };
    ...
    pub async fn add_to_blacklist(&self, blocked_user_id: u64) -> Result<serde_json::Value> {
        ...
        let response = client.call_rpc_typed(routes::blacklist::ADD, request).await?;
    ...
    pub async fn remove_from_blacklist(&self, blocked_user_id: u64) -> Result<serde_json::Value> {
        ...
        let response = client.call_rpc_typed(routes::blacklist::REMOVE, request).await?;
    ...
    pub async fn get_blacklist(&self) -> Result<serde_json::Value> {
        ...
        let request = BlacklistListRequest {
            user_id: user_id,
        };
        
        let response = client.call_rpc_typed(routes::blacklist::LIST, request).await?;
    ...
    async fn emit_channel_list_reset(&self) {
        ...
        tokio::spawn(async move {
            ...
            for conv in channels {
                // 获取频道信息
                let channel = storage.get_channel(conv.channel_id, conv.channel_type).await.ok().flatten();
                
                // 获取群组成员数量（如果是群聊）
                let member_count = if conv.channel_type == 2 {
                    storage.get_group_members(conv.channel_id, None, None)
                        .await
            ...
            let conv_event = crate::events::SDKEvent::ChannelListUpdate {
                update_kind: ChannelListUpdateKind::Reset {
                    channels: entries,
                },
                timestamp,
            };
    ...
    pub async fn get_channel_list_entries(
        ...
        // 获取当前用户ID（用于判断私聊的对方）
        let current_user_id = self.user_id().await;
        ...
        for conv in channels {
            // 获取频道信息
            let channel = self.storage().get_channel(conv.channel_id, conv.channel_type).await.ok().flatten();
            
            // 获取群组成员数量（如果是群聊）
            let member_count = if conv.channel_type == 2 {
                self.storage().get_group_members(conv.channel_id, None, None)
                    .await
                    .map(|members| members.len() as u32)
                    .unwrap_or(0)
            } else {
            ...
            // 构建 ChannelListEntry（传入当前用户ID用于私聊名称处理）
            let entry = Self::build_channel_list_entry_with_user_id(
                &conv,
                channel.as_ref(),
                member_count,
                None, // 列表查询不需要 latest_event
                current_user_id,
            );
    ...
    /// 辅助方法：构建 ChannelListEntry（带用户ID，用于私聊名称处理）
    pub(crate) fn build_channel_list_entry_with_user_id(
        ...
        if (conv.channel_type == 0 || conv.channel_type == 1) && (entry.name.is_empty() || entry.name.starts_with("Channel ")) {
        ...
        if (conv.channel_type == 0 || conv.channel_type == 1) && (entry.name == "User " || entry.name == "User") {
            if let Some(ch) = channel {
                entry.name = peer_display(ch);
            } else {
                entry.name = if !conv.username.is_empty() {
    ...
    pub(crate) fn build_channel_list_entry(
        ...
        use crate::events::ChannelListEntry;
        ...
        let is_favourite = channel
            .map(|c| c.save == 1)
            .unwrap_or(false);
        
        let notifications = channel
            .map(|c| if c.mute == 1 { 0 } else { conv.unread_count as u32 })
            .unwrap_or(conv.unread_count as u32);
        
        let marked_unread = channel
            .map(|c| c.top == 1)
            .unwrap_or(false);
    ...
    pub async fn mark_channel_read(&self, channel_id: u64, channel_type: i32) -> Result<()> {
        ...
        if channel_type != 1 && channel_type != 2 {
            return Err(PrivchatSDKError::InvalidInput(
                format!("无效的 channel_type: {}，必须是 1(私聊) 或 2(群聊)", channel_type)
            ));
        }
        
        debug!("标记会话已读: channel_id={}, channel_type={}", channel_id, channel_type);
        
        // 调用存储层标记已读
        self.storage.mark_channel_read(channel_id, channel_type).await?;
        
        // 发送会话列表更新事件
        self.emit_channel_list_update(channel_id, channel_type).await;
    ...
    #[allow(dead_code)]
    fn parse_channel_from_json(&self, value: &serde_json::Value, current_user_id: u64) -> Result<crate::storage::entities::Channel> {
        let now = chrono::Utc::now().timestamp_millis();
        
        // 支持两种字段名：id 或 channel_id（服务器返回的是 id）
        let channel_id = value.get("id")
            .and_then(|v| v.as_u64())
        ...
        let channel_type = value.get("channel_type")
            .and_then(|v| {
        ...
        let other_uid = (channel_type == 0).then(|| match (direct_user1_id, direct_user2_id) {
        ...
        let (channel_name_from_members, username_from_members) = if channel_type == 0 {
            let (name, uid_str) = if let Some(uid) = other_uid {
                let display_name = value.get("members")
        ...
        Ok(crate::storage::entities::Channel {
            ...
            channel_id,
            channel_type,
            last_local_message_id: value.get("last_message_id")
                .and_then(|v| v.as_u64())
                .or_else(|| value.get("last_local_message_id").and_then(|v| v.as_u64()))
                .unwrap_or(0),
            last_msg_timestamp: value.get("last_message_at")
                .and_then(|v| {
                    // 支持 DateTime 字符串或时间戳
            ...
            extra: value.get("extra")
                ...
                .unwrap_or("")
                .to_string(),
            last_msg_pts: value.get("last_msg_pts")  // ⭐ last_msg_seq -> last_msg_pts
                .and_then(|v| v.as_i64())
                .unwrap_or(0),
            // 频道信息字段（使用默认值）
            show_nick: value.get("show_nick")
                .and_then(|v| v.as_i64())
                .unwrap_or(0) as i32,
            ...
            channel_remark: value.get("channel_remark")
                .and_then(|v| v.as_str())
                .unwrap_or("")
                .to_string(),
            top: value.get("top")
            ...
            online: value.get("online")
                .and_then(|v| v.as_i64())
                .unwrap_or(0) as i16,
            last_offline: value.get("last_offline")
                .and_then(|v| v.as_i64())
                .unwrap_or(0),
            ...
            updated_at: value.get("updated_at")
                .and_then(|v| v.as_i64())
                .unwrap_or(now),
            avatar_cache_key: value.get("avatar_cache_key")
                .and_then(|v| v.as_str())
                .unwrap_or("")
                .to_string(),
            remote_extra: value.get("remote_extra")
                .and_then(|v| v.as_str())
                .map(|s| s.to_string()),
            flame: value.get("flame")
                .and_then(|v| v.as_i64())
                .unwrap_or(0) as i16,
            flame_second: value.get("flame_second")
                .and_then(|v| v.as_i64())
                .unwrap_or(0) as i32,
            device_flag: value.get("device_flag")
                .and_then(|v| v.as_i64())
                .unwrap_or(0) as i32,
            parent_channel_id: value.get("parent_channel_id")
                .and_then(|v| v.as_u64())
                .unwrap_or(0),
            parent_channel_type: value.get("parent_channel_type")
                .and_then(|v| v.as_i64())
                .unwrap_or(0) as i32,
        })
    }
    ...
    pub async fn pin_channel(&self, channel_id: u64, pin: bool) -> Result<bool> {
        ...
        let storage = self.storage.clone();
        let channel_type = storage.get_channel(channel_id, 1).await
            .ok()
            .flatten()
    ...
    pub async fn hide_channel(&self, channel_id: u64) -> Result<bool> {
        ...
        let storage = self.storage.clone();
        let _channel_type = storage.get_channel(channel_id, 1).await
            .ok()
            .flatten()
            .map(|ch| ch.channel_type)
            .unwrap_or(1); // 默认为私聊
        ...
        let _: bool = client.call_rpc_typed(routes::channel::HIDE, request).await?;
        
        info!("✅ 频道已隐藏: channel={}", channel_id);
        
        // 发送会话列表更新事件
        use crate::events::{SDKEvent, ChannelListUpdateKind};
        use std::time::{SystemTime, UNIX_EPOCH};
    ...
    pub async fn mute_channel(&self, channel_id: u64, muted: bool) -> Result<bool> {
        ...
        let _: bool = client.call_rpc_typed(routes::channel::MUTE, request).await?;
        
        info!("✅ 频道已{}: channel={}", if muted { "静音" } else { "取消静音" }, channel_id);
    ...
    pub async fn channel_unread_stats(&self, channel_id: u64) -> Result<crate::events::UnreadStats> {
        self.check_initialized().await?;
        
        let _user_id = self.user_id().await
            .ok_or(PrivchatSDKError::NotConnected)?;
    ...
    /// - `route`: RPC路由路径
    /// - `params`: 请求参数（JSON格式）
    pub async fn rpc_call(&self, route: &str, params: serde_json::Value) -> Result<serde_json::Value> {
        self.check_initialized().await?;
        self.check_connected().await?;
    ...
    async fn notify_send_status_enqueued(&self, task: &crate::storage::queue::send_task::SendTask) {
        ...
        let timestamp = SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_secs();
        self.event_manager.emit(SDKEvent::SendStatusUpdate {
    ...
    /// # 返回
    /// 同步状态
    pub async fn sync_channel(&self, channel_id: u64, channel_type: u8) -> Result<crate::sync::ChannelSyncState> {
        self.check_initialized().await?;
        
        let sync_engine = self.sync_engine.read().await;
        let sync_engine = sync_engine.as_ref()
            .ok_or_else(|| PrivchatSDKError::NotConnected)?;
    ...
    pub async fn sync_all_channels(&self) -> Result<Vec<crate::sync::ChannelSyncState>> {
        ...
        // 获取所有会话
        let channels = self.storage.get_channels(&crate::storage::entities::ChannelQuery::default()).await?;
        
        let channels: Vec<(u64, u8)> = channels.iter()
    ...
    /// # 返回
    /// 频道的本地 pts 和服务器 pts
    pub async fn get_channel_sync_state(&self, channel_id: u64, channel_type: u8) -> Result<(u64, u64)> {
        self.check_initialized().await?;
    ...
    pub async fn needs_sync(&self, channel_id: u64, channel_type: u8) -> Result<bool> {
        let (local_pts, server_pts) = self.get_channel_sync_state(channel_id, channel_type).await?;
        Ok(local_pts < server_pts)
    }
    ...
    pub async fn channel_send_queue_set_enabled(&self, channel_id: u64, enabled: bool) -> Result<()> {
        // 当前实现中，所有会话共享同一个发送队列
        // 未来可以实现会话级别的队列控制
    ...
    pub async fn retry_message(&self, message_id: i64) -> Result<()> {
        ...
        if let Some(mut task) = queue_manager.load_task(&id_str)? {
            ...
            let new_local_message_id = self.snowflake.next_id()
                .map_err(|e| PrivchatSDKError::Other(format!("生成 local_message_id 失败: {:?}", e)))?;
            ...
            info!("✅ 消息已重新入队: message.id={}, new_local_message_id={}", task.id, new_local_message_id);
        } else {
            return Err(PrivchatSDKError::InvalidInput(
                format!("未找到 message.id={} 对应的消息", message_id)
            ));
        }
    ...
    pub async fn set_channel_favourite(&self, channel_id: u64, favourite: bool) -> Result<()> {
        ...
        let channel = channel.ok_or_else(|| PrivchatSDKError::InvalidInput(
            format!("会话不存在: channel_id={}", channel_id)
        ))?;
        ...
        self.storage.update_channel_save(channel_id, channel_type, save).await?;
        
        // 触发会话列表更新
        self.emit_channel_list_update(channel_id, channel_type).await;
        
        info!("✅ 会话收藏状态已更新: channel_id={}, favourite={}", channel_id, favourite);
        Ok(())
    }
    ...
    /// # 返回
    /// - `Ok(())`: 操作成功
    pub async fn set_channel_low_priority(&self, channel_id: u64, low_priority: bool) -> Result<()> {
        self.check_initialized().await?;
        
        // 获取会话（先尝试私聊，再尝试群聊）
        let channel = if let Ok(Some(conv)) = self.storage.get_channel_by_channel(channel_id, 1).await {
            Some(conv)
        ...
        // 解析现有的 extra JSON
        let mut extra: serde_json::Value = serde_json::from_str(&channel.extra)
        ...
        self.storage.update_channel_extra(channel_id, channel_type, extra_str).await?;
        
        // 触发会话列表更新
        self.emit_channel_list_update(channel_id, channel.channel_type).await;
        
        info!("✅ 会话优先级状态已更新: channel_id={}, low_priority={}", channel_id, low_priority);
        Ok(())
    }
    ...
    pub async fn channel_tags(&self, channel_id: u64) -> Result<crate::events::ChannelTags> {
        ...
        let low_priority = channel.as_ref()
            .and_then(|c| {
                serde_json::from_str::<serde_json::Value>(&c.extra).ok()
            })
            .and_then(|extra| extra.get("low_priority")?.as_bool())
            .unwrap_or(false);
    ...
    pub async fn channel_notification_mode(&self, channel_id: u64) -> Result<crate::events::NotificationMode> {
        self.check_initialized().await?;
    ...
    pub async fn set_channel_notification_mode(
        ...
        self.storage.update_channel_mute(channel_id, channel_type, mute).await?;
        
        // 触发会话列表更新
        self.emit_channel_list_update(channel_id, channel_type).await;
        
        info!("✅ 会话通知模式已更新: channel_id={}, mode={:?}", channel_id, mode);
        Ok(())
    }
    ...
    pub async fn list_my_devices(&self) -> Result<Vec<crate::events::DeviceSummary>> {
        ...
        let device_id = connection_state.user
            .as_ref()
        ...
        let devices: Vec<crate::events::DeviceSummary> = response.devices
            .into_iter()
            .map(|item| {
                crate::events::DeviceSummary {
    ...
    pub async fn update_device_push_state(
        ...
        info!("✅ 设备推送状态已更新: device_id={}, apns_armed={}, user_push_enabled={}", 
    ...
    pub async fn get_device_push_status(
        ...
        info!("✅ 设备推送状态查询成功: 设备数量={}, user_push_enabled={}", 
              response.devices.len(), response.user_push_enabled);
    ...
    /// ```
    pub async fn register_lifecycle_hook(&self, hook: Arc<dyn crate::lifecycle::LifecycleHook>) -> Result<()> {
        self.check_initialized().await?;
    ...
    pub async fn dm_peer_user_id(&self, channel_id: u64) -> Result<Option<u64>> {
        ...
        let peer_user_id = channel.username.parse::<u64>()
            .ok()
            .map(|id| {
                info!("✅ 获取私聊对等用户ID成功: channel_id={}, peer_user_id={}", channel_id, id);
                id
            });
...
impl PrivchatSDK {
    ...
    pub fn mark_as_read_blocking(&self, session_id: &str, message_id: u64) -> Result<()> {
        if let Some(rt) = &self.sync_runtime {
            let channel_id = session_id.parse::<u64>()
                .map_err(|_| PrivchatSDKError::InvalidInput("无效的 session_id".to_string()))?;
            rt.block_on(async {
                self.mark_as_read(channel_id, message_id).await
            })
        } else {
            Err(PrivchatSDKError::Runtime("同步运行时未初始化".to_string()))
        }
...
mod tests {
    ...
    async fn test_sync_friends_after_reconnect() {
        ...
        let count1 = sdk.sync_entities(crate::sync::EntityType::Friend, None).await.expect("first sync_entities friend");
        let friends_count1 = sdk.get_friends_count().await.expect("get_friends_count after first sync");
        assert_eq!(
            count1 as u32,
            friends_count1,
        ...
        let count2 = sdk.sync_entities(crate::sync::EntityType::Friend, None).await.expect("second sync_entities friend");
        let friends_count2 = sdk.get_friends_count().await.expect("get_friends_count after second sync");
        assert_eq!(
            count2 as u32,
            friends_count2,
