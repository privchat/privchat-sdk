...
impl TypingManager {
    ...
    /// 清理指定用户的所有输入状态
    pub fn clear_user_typing_states(&self, user_id: &str) -> crate::Result<usize> {
        let mut removed_count = 0;
...
mod tests {
    ...
    async fn test_typing_state_management() {
        ...
        let event = manager.update_typing_state(
            "user1", "channel1", 1, true, Some("session1")
        ).unwrap();

        assert_eq!(event.user_id, "user1");
        assert_eq!(event.channel_id, "channel1");
        assert!(event.is_typing);

        // 获取正在输入的用户
        let typing_users = manager.get_typing_users("channel1", 1).unwrap();
        assert_eq!(typing_users.len(), 1);
        assert_eq!(typing_users[0].user_id, "user1");

        // 设置用户停止输入
        let event = manager.update_typing_state(
            "user1", "channel1", 1, false, Some("session1")
        ).unwrap();
        assert!(!event.is_typing);
    ...
    async fn test_cleanup_expired_states() {
        ...
        // 添加多个用户的输入状态
        manager.update_typing_state("user1", "channel1", 1, true, None).unwrap();
        manager.update_typing_state("user2", "channel1", 1, true, None).unwrap();
        manager.update_typing_state("user3", "channel2", 1, true, None).unwrap();
        ...
        // 所有频道都应该没有正在输入的用户
        let typing_users1 = manager.get_typing_users("channel1", 1).unwrap();
        let typing_users2 = manager.get_typing_users("channel2", 1).unwrap();
    ...
    async fn test_multiple_users_typing() {
        ...
        manager.update_typing_state("user1", "channel1", 1, true, None).unwrap();
        manager.update_typing_state("user2", "channel1", 1, true, None).unwrap();
        manager.update_typing_state("user3", "channel1", 1, true, None).unwrap();
        ...
        // 一个用户停止输入
        manager.update_typing_state("user2", "channel1", 1, false, None).unwrap();
    ...
    async fn test_database_persistence() {
        ...
        // 清理用户状态
        let removed = manager.clear_user_typing_states("user1").unwrap();
