...
impl ReceiveTask {
    ...
    /// 生成任务ID
    fn generate_task_id(server_msg_id: &str, sequence_id: u64) -> String {
...
impl ReceiveTaskBatch {
    /// 创建新批次
    pub fn new(conversation_key: String, max_size: usize, timeout_seconds: u64) -> Self {
    ...
    pub fn add_task(&mut self, mut task: ReceiveTask) -> Result<()> {
        ...
        if task.conversation_key() != self.conversation_key {
            return Err(PrivchatSDKError::InvalidInput(
                "Task conversation key doesn't match batch".to_string()
            ));
        }
...
mod tests {
    ...
    fn test_receive_task_creation() {
        ...
        assert_eq!(task.server_msg_id, "server_msg_789");
    ...
    fn test_task_dedup_key() {
        ...
        assert_eq!(task.dedup_key(), "server_msg_789_12345");
    }

    #[test]
    fn test_task_conversation_key() {
        let message_data = create_test_message_data();
        let task = ReceiveTask::new(
            message_data,
            12345,
        ...
        assert_eq!(task.conversation_key(), "channel_456_1");
    }
    ...
    fn test_receive_task_batch() {
        let mut batch = ReceiveTaskBatch::new(
            "channel_456_1".to_string(),
            5,
            30,
        );

        assert_eq!(batch.conversation_key, "channel_456_1");
