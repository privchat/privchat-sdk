use crate::storage::queue::receive_task::{ReceiveTask, ReceiveTaskStatus, MessageSource};
use crate::storage::queue::receive_queue::{ReceiveQueueManager, ReceiveQueueConfig};
...
impl ReceiveConsumerRunner {
    ...
    async fn process_single_task(
        ...
        state_manager.update_message_state(
            &task.message_data.client_msg_no,
    ...
    fn convert_task_to_message(task: &ReceiveTask) -> Result<Message> {
        ...
        Ok(Message {
            client_seq: None,
            message_id: Some(task.server_msg_id.clone()),
            message_seq: task.sequence_id as i64,
            channel_id: task.message_data.channel_id.clone(),
            channel_type: task.message_data.channel_type,
            timestamp: Some(now),
            from_uid: task.message_data.from_uid.clone(),
            ...
            client_msg_no: task.message_data.client_msg_no.clone(),
            is_deleted: 0,
            setting: 0,
...
mod tests {
    ...
    fn create_test_message_data() -> MessageData {
        MessageData {
            client_msg_no: "test_client_123".to_string(),
    ...
    async fn test_task_to_message_conversion() {
        ...
        let task = ReceiveTask::new(
            message_data.clone(),
            12345,
            "server_msg_789".to_string(),
            MessageSource::RealTime,
        );

        let message = ReceiveConsumerRunner::convert_task_to_message(&task).unwrap();
        
        assert_eq!(message.client_msg_no, task.message_data.client_msg_no);
        assert_eq!(message.message_id, Some(task.server_msg_id));
        assert_eq!(message.channel_id, task.message_data.channel_id);
        assert_eq!(message.message_seq, task.sequence_id as i64);
