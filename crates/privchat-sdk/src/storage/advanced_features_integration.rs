...
impl AdvancedFeaturesIntegration {
    ...
    /// 更新会话已读状态（带事件发布）
    pub async fn update_conversation_read_state(
        &self,
        channel_id: &str,
        channel_type: i32,
        user_id: &str,
        ...
        // 调用底层功能
        let read_state = self.features_manager.update_conversation_read_state(
    ...
    /// 检查消息是否可以撤回
    pub async fn can_revoke_message(&self, message_id: &str, user_id: &str) -> Result<bool> {
    ...
    /// 检查消息是否可以编辑
    pub async fn can_edit_message(&self, message_id: &str, user_id: &str) -> Result<bool> {
        self.features_manager.can_edit_message(message_id, user_id)
    }

    /// 获取消息编辑历史
    pub async fn get_message_edit_history(&self, message_id: &str) -> Result<Vec<MessageEditEvent>> {
    ...
    /// 获取消息的已读回执列表
    pub async fn get_message_read_receipts(&self, message_id: &str) -> Result<Vec<ReadReceiptEvent>> {
        self.features_manager.get_message_read_receipts(message_id)
    }

    /// 获取会话的未读消息数量
    pub async fn get_unread_count(&self, channel_id: &str, channel_type: i32, user_id: &str) -> Result<i32> {
        self.features_manager.get_unread_count(channel_id, channel_type, user_id)
    }

    /// 获取会话的所有已读状态
    pub async fn get_conversation_read_states(
        &self,
        channel_id: &str,
        channel_type: i32,
    ) -> Result<Vec<crate::storage::advanced_features::ConversationReadState>> {
        self.features_manager.get_conversation_read_states(channel_id, channel_type)
    }
