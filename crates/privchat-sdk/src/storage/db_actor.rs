...
impl DbActor {
    ...
    fn new(receiver: Receiver<DbCommand>, assets_path: Option<PathBuf>) -> Self {
        ...
        info!("🚀 [Thread {:?}] DbActor 已启动, assets_path={:?}", thread_id, assets_path);
    ...
    fn migrate_database(&self, conn: &mut Connection, uid: &str) -> Result<()> {
        if self.assets_path.is_some() {
            ...
            match current_version {
                None => {
                    info!("🆕 [DbActor Thread {:?}] 全新数据库，执行外部 assets: uid={}", self.thread_id, uid);
                    self.execute_init_sql(conn)?;
                }
                Some(version) => {
                    info!("📌 [DbActor Thread {:?}] 当前版本: {}, 检查增量: uid={}", self.thread_id, version, uid);
                    self.execute_migrations(conn, &version)?;
                }
            }
        } else {
            // 内置：refinery 自动管理（无需手写 BUILTIN_MIGRATIONS）
            info!("📦 [DbActor Thread {:?}] 使用 refinery embedded migrations: uid={}", self.thread_id, uid);
    ...
    fn execute_init_sql(&self, conn: &Connection) -> Result<()> {
        ...
        info!("🔄 [DbActor Thread {:?}] 开始初始化数据库，共 {} 个SQL文件", 
             self.thread_id, all_migrations.len());
        
        // 按顺序执行所有 SQL 文件
        for (version, sql_content) in all_migrations {
            info!("📄 [DbActor Thread {:?}] 执行初始化SQL: {}", self.thread_id, version);
            
            conn.execute_batch(&sql_content)
                .map_err(|e| {
                    error!("❌ [DbActor Thread {:?}] 执行SQL失败: {}, error={}", self.thread_id, version, e);
            ...
            info!("✅ [DbActor Thread {:?}] SQL执行成功: {}", self.thread_id, version);
        }
    ...
    fn execute_migrations(&self, conn: &Connection, current_version: &str) -> Result<()> {
        ...
        if pending_migrations.is_empty() {
            info!("✓ [DbActor Thread {:?}] 数据库已是最新版本: {}", self.thread_id, current_version);
            return Ok(());
        }
        
        info!("🔄 [DbActor Thread {:?}] 发现 {} 个待执行的迁移脚本", self.thread_id, pending_migrations.len());
        
        // 依次执行迁移
        for (version, sql_content) in pending_migrations {
            info!("📄 [DbActor Thread {:?}] 执行迁移: {}", self.thread_id, version);
            
            conn.execute_batch(&sql_content)
                .map_err(|e| {
                    error!("❌ [DbActor Thread {:?}] 执行迁移失败: {}, error={}", self.thread_id, version, e);
            ...
            info!("✅ [DbActor Thread {:?}] 迁移执行成功: {}", self.thread_id, version);
        }
    ...
    fn scan_all_migration_files(&self) -> Result<Vec<(String, String)>> {
        let assets_path = self.assets_path.as_ref().expect("scan_all_migration_files 仅在 assets_path 模式下调用");
        info!(
                        ...
                        if file_name.ends_with(".sql") {
                            ...
                            if Self::is_valid_version(&version) {
                                let sql_content = std::fs::read_to_string(&path)
                                    .map_err(|e| PrivchatSDKError::IO(format!(
                                        "读取 SQL 文件失败 {}: {}",
                                        file_name, e
                                    )))?;
                                info!(
    ...
    fn run(mut self) {
        ...
        while let Ok(command) = self.receiver.recv() {
            match command {
                ...
                DbCommand::InitUser { uid, db_path, respond_to } => {
                    let result = self.handle_init_user(&uid, &db_path);
                    let _ = respond_to.send(result);
                }
                
                DbCommand::Execute { uid, sql, params, respond_to } => {
                    let result = self.handle_execute(&uid, &sql, &params);
                    let _ = respond_to.send(result);
                }
                
                DbCommand::UpdateMessageStatus { uid, id, status, respond_to } => {
                    let result = self.handle_update_message_status(&uid, id, status);
                    let _ = respond_to.send(result);
                }
                
                DbCommand::UpdateMessageServerId { uid, id, server_message_id, respond_to } => {
                    let result = self.handle_update_message_server_id(&uid, id, server_message_id);
                    let _ = respond_to.send(result);
                }
                
                DbCommand::SaveReceivedMessage { uid, message, respond_to } => {
                    let result = self.handle_save_received_message(&uid, &message);
                    let _ = respond_to.send(result);
                }
                
                DbCommand::Query { uid, sql, params, respond_to } => {
                    let result = self.handle_query(&uid, &sql, &params);
                    let _ = respond_to.send(result);
                }
                
                DbCommand::GetMessageById { uid, id, respond_to } => {
                    let result = self.handle_get_message_by_id(&uid, id);
                    let _ = respond_to.send(result);
                }
                
                DbCommand::GetMessageChannelId { uid, id, respond_to } => {
                    let result = self.handle_get_message_channel_id(&uid, id);
                    let _ = respond_to.send(result);
                }
                
                DbCommand::GetMessagesBefore { uid, channel_id, before_id, limit, respond_to } => {
                    let result = self.handle_get_messages_before(&uid, channel_id, before_id, limit);
                    let _ = respond_to.send(result);
                }
                
                DbCommand::GetEarliestId { uid, channel_id, respond_to } => {
                    let result = self.handle_get_earliest_id(&uid, channel_id);
                    let _ = respond_to.send(result);
                }
                
                DbCommand::GetMessagesAfter { uid, channel_id, after_id, limit, respond_to } => {
                    let result = self.handle_get_messages_after(&uid, channel_id, after_id, limit);
                    let _ = respond_to.send(result);
                }
                
                DbCommand::SendMessage { uid, channel_id, channel_type, from_uid, content, message_type, respond_to } => {
                    let result = self.handle_send_message(&uid, channel_id, channel_type, from_uid, &content, message_type);
                    let _ = respond_to.send(result);
                }
                
                DbCommand::RevokeMessage { uid, id, revoker_id, respond_to } => {
                    let result = self.handle_revoke_message(&uid, id, revoker_id);
                    let _ = respond_to.send(result);
                }
                
                DbCommand::EditMessage { uid, id, new_content, respond_to } => {
                    let result = self.handle_edit_message(&uid, id, &new_content);
                    let _ = respond_to.send(result);
                }
                
                DbCommand::DeleteMessage { uid, id, respond_to } => {
                    let result = self.handle_delete_message(&uid, id);
                    let _ = respond_to.send(result);
                }
                
                DbCommand::UpdateMessageContent { uid, id, new_content, respond_to } => {
                    let result = self.handle_update_message_content(&uid, id, &new_content);
                    let _ = respond_to.send(result);
                }
                
                DbCommand::AddMessageReaction { uid, id, user_id, reaction, respond_to } => {
                    let result = self.handle_add_message_reaction(&uid, id, user_id, &reaction);
                    let _ = respond_to.send(result);
                }
                
                DbCommand::SaveChannel { uid, channel, respond_to } => {
                ...
                DbCommand::GetChannelByChannel { uid, channel_id, channel_type, respond_to } => {
                    let result = self.handle_get_channel_by_channel(&uid, channel_id, channel_type);
                    let _ = respond_to.send(result);
                }
                
                DbCommand::GetDirectChannelById { uid, channel_id, respond_to } => {
                    let result = self.handle_get_direct_channel_by_id(&uid, channel_id);
                    let _ = respond_to.send(result);
                }
                
                DbCommand::UpdateChannelPts { uid, channel_id, channel_type, new_pts, respond_to } => {
                    let result = self.handle_update_channel_pts(&uid, channel_id, channel_type, new_pts);
                    let _ = respond_to.send(result);
                }
                
                DbCommand::FindChannelIdByUser { uid, target_user_id, respond_to } => {
                    let result = self.handle_find_channel_id_by_user(&uid, target_user_id);
                    let _ = respond_to.send(result);
                }
                
                DbCommand::UpdateChannelSave { uid, channel_id, channel_type, save, respond_to } => {
                    let result = self.handle_update_channel_save(&uid, channel_id, channel_type, save);
                    let _ = respond_to.send(result);
                }
                
                DbCommand::UpdateChannelMute { uid, channel_id, channel_type, mute, respond_to } => {
                    let result = self.handle_update_channel_mute(&uid, channel_id, channel_type, mute);
                    let _ = respond_to.send(result);
                }
                
                DbCommand::UpdateChannelExtra { uid, channel_id, channel_type, extra, respond_to } => {
                    let result = self.handle_update_channel_extra(&uid, channel_id, channel_type, &extra);
                    let _ = respond_to.send(result);
                }
                
                // ========== User / Group / GroupMember 命令处理 ==========
                
                DbCommand::SaveUser { uid, user, respond_to } => {
                    let result = self.handle_save_user(&uid, &user);
                    let _ = respond_to.send(result);
                }
                
                DbCommand::SaveUsers { uid, users, respond_to } => {
                    let result = self.handle_save_users(&uid, &users);
                    let _ = respond_to.send(result);
                }
                
                DbCommand::GetUser { uid, user_id, respond_to } => {
                    let result = self.handle_get_user(&uid, user_id);
                    let _ = respond_to.send(result);
                }
                
                DbCommand::GetUsersByIds { uid, ids, respond_to } => {
                ...
                DbCommand::SaveFriend { uid, friend, respond_to } => {
                    let result = self.handle_save_friend(&uid, &friend);
                    let _ = respond_to.send(result);
                }
                
                DbCommand::SaveFriends { uid, friends, respond_to } => {
                    let result = self.handle_save_friends(&uid, &friends);
                    let _ = respond_to.send(result);
                }
                
                DbCommand::GetFriends { uid, limit, offset, respond_to } => {
                ...
                DbCommand::DeleteFriend { uid, user_id, respond_to } => {
                    let result = self.handle_delete_friend(&uid, user_id);
                    let _ = respond_to.send(result);
                }
                
                DbCommand::SaveChannelMember { uid, member, respond_to } => {
                    let result = self.handle_save_channel_member(&uid, &member);
                    let _ = respond_to.send(result);
                }
                
                DbCommand::SaveChannelMembers { uid, members, respond_to } => {
                    let result = self.handle_save_channel_members(&uid, &members);
                    let _ = respond_to.send(result);
                }
                
                DbCommand::GetGroupMembers { uid, group_id, limit, offset, respond_to } => {
                    let result = self.handle_get_group_members(&uid, group_id, limit, offset);
                    let _ = respond_to.send(result);
                }
                
                DbCommand::DeleteChannelMember { uid, channel_id, channel_type, member_uid, respond_to } => {
                    let result = self.handle_delete_channel_member(&uid, channel_id, channel_type, member_uid);
                    let _ = respond_to.send(result);
                }
                
                DbCommand::GetGroups { uid, limit, offset, respond_to } => {
                    let result = self.handle_get_groups(&uid, limit, offset);
                    let _ = respond_to.send(result);
                }
                
                DbCommand::GetGroup { uid, group_id, respond_to } => {
                    let result = self.handle_get_group(&uid, group_id);
                    let _ = respond_to.send(result);
                }
                
                DbCommand::SaveGroups { uid, groups, respond_to } => {
    ...
    fn handle_init_user(&mut self, uid: &str, db_path: &PathBuf) -> Result<()> {
        info!("📥 [DbActor Thread {:?}] 接收命令: InitUser(uid={}, path={})", 
               self.thread_id, uid, db_path.display());
        
        // 检查是否已经初始化
        if self.connections.contains_key(uid) {
            info!("⚠️  [DbActor Thread {:?}] 用户已初始化，跳过: uid={}", self.thread_id, uid);
            return Ok(());
        }
        
        info!("🔨 [DbActor Thread {:?}] 开始打开数据库: uid={}", self.thread_id, uid);
        
        // 打开数据库
        let mut conn = Connection::open(db_path)
            .map_err(|e| {
                error!("❌ [DbActor Thread {:?}] 打开数据库失败: uid={}, error={}", self.thread_id, uid, e);
                PrivchatSDKError::Database(format!("打开数据库失败: {}", e))
            })?;
        
        info!("🔐 [DbActor Thread {:?}] 设置数据库加密: uid={}", self.thread_id, uid);
        
        // 🔐 设置 SQLCipher 加密密钥
        let encryption_key = Self::derive_encryption_key(uid);
        conn.pragma_update(None, "key", &encryption_key)
            .map_err(|e| {
                error!("❌ [DbActor Thread {:?}] 设置加密密钥失败: uid={}, error={}", self.thread_id, uid, e);
        ...
        if self.assets_path.is_none() {
            ...
            info!("🔧 [DbActor Thread {:?}] 设置 PRAGMA（外部 assets 路径）: uid={}", self.thread_id, uid);
            crate::storage::migrate::enable_pragmas(&conn)?;
            info!("🔄 [DbActor Thread {:?}] 执行外部 assets 迁移: uid={}", self.thread_id, uid);
            self.migrate_database(&mut conn, uid)?;
        }
        
        info!("✅ [DbActor Thread {:?}] 数据库迁移完成: uid={}", self.thread_id, uid);
        ...
        if table_count < 3 {
            error!("❌ [DbActor Thread {:?}] 关键表缺失: uid={}, found={}/3", self.thread_id, uid, table_count);
            return Err(PrivchatSDKError::Database("数据库初始化不完整：关键表缺失".to_string()));
        }
        
        info!("✅ [DbActor Thread {:?}] Schema验证通过: uid={}, 核心表数量={}/3", self.thread_id, uid, table_count);
        
        info!("💾 [DbActor Thread {:?}] 保存连接到连接池: uid={} (保存前连接数={})", 
             self.thread_id, uid, self.connections.len());
        
        // 保存连接
        self.connections.insert(uid.to_string(), conn);
        
        info!("✅ [DbActor Thread {:?}] 用户数据库初始化完成: uid={} (保存后连接数={}, keys={:?})", 
             self.thread_id, uid, self.connections.len(), self.connections.keys().collect::<Vec<_>>());
        
        Ok(())
    }
    
    /// 处理：执行 SQL（通用）
    fn handle_execute(&mut self, uid: &str, sql: &str, params: &[rusqlite::types::Value]) -> Result<usize> {
        let conn = self.connections.get(uid)
    ...
    fn handle_update_message_status(&mut self, uid: &str, id: i64, status: i32) -> Result<()> {
        info!("📥 [DbActor Thread {:?}] 接收命令: UpdateMessageStatus(uid={}, id={}, status={})", 
               self.thread_id, uid, id, status);
        
        // 调试：列出所有连接
        let available_uids: Vec<&String> = self.connections.keys().collect();
        info!("🗂️  [DbActor Thread {:?}] 当前连接池状态: {} 个连接, uids={:?}", 
             self.thread_id, self.connections.len(), available_uids);
        
        if !self.connections.contains_key(uid) {
            error!("❌ [DbActor Thread {:?}] 用户数据库不存在! requested_uid={}, available_uids={:?}", 
                  self.thread_id, uid, available_uids);
            return Err(PrivchatSDKError::Database(format!(
                "用户数据库不存在: requested={}, available={:?}", uid, available_uids
            )));
        }
        
        info!("✓ [DbActor Thread {:?}] 找到数据库连接: uid={}", self.thread_id, uid);
        ...
        match conn.query_row::<i32, _, _>(
            ...
            Ok(count) => {
                if count == 0 {
                    error!("❌ [DbActor Thread {:?}] message 表不存在！uid={}", self.thread_id, uid);
                    // 列出所有表
                    if let Ok(mut stmt) = conn.prepare("SELECT name FROM sqlite_master WHERE type='table'") {
                        if let Ok(rows) = stmt.query_map([], |row| row.get::<_, String>(0)) {
                            ...
                            error!("📋 [DbActor Thread {:?}] 现有表列表: {:?}", self.thread_id, tables);
                        }
                    }
                    return Err(PrivchatSDKError::Database(
                        format!("message 表不存在，数据库可能未正确初始化")
                    ));
                } else {
                    info!("✓ [DbActor Thread {:?}] message 表存在: uid={}", self.thread_id, uid);
                }
            }
            Err(e) => {
                error!("❌ [DbActor Thread {:?}] 查询表失败: uid={}, error={}", self.thread_id, uid, e);
            }
        }
        ...
        let affected = conn.execute(
            ...
            error!("❌ [DbActor Thread {:?}] SQL 执行失败: uid={}, error={}", self.thread_id, uid, e);
            PrivchatSDKError::Database(format!("更新消息状态失败: {}", e))
        })?;
        
        info!("✅ [DbActor Thread {:?}] 更新成功: uid={}, affected={} rows", 
    ...
    fn handle_update_message_server_id(&mut self, uid: &str, id: i64, server_message_id: u64) -> Result<()> {
        info!("🔍 [DbActor] 更新 message_id: uid={}, id={}, server_message_id={}", uid, id, server_message_id);
    ...
    fn handle_save_received_message(&mut self, uid: &str, message: &Message) -> Result<i64> {
        let conn = self.connections.get(uid)
            .ok_or_else(|| PrivchatSDKError::Database(format!("用户数据库不存在: {}", uid)))?;
        ...
        let row_id = if let Some(msg_id) = message.server_message_id {
            ...
            if let Some(id) = existing_id {
                debug!("[Rust SDK] 💾 跳过重复(已存在): channel_id={}, message_id={}, 已有 row_id={}", message.channel_id, msg_id, id);
                id
            } else {
                use crate::storage::dao::MessageDao;
                let message_dao = MessageDao::new(conn);
                let id = message_dao.insert(message)?;
                debug!("[Rust SDK] 💾 写入本地 DB: channel_id={}, message_id={:?}, row_id={}", message.channel_id, message.server_message_id, id);
                id
            }
        } else {
            use crate::storage::dao::MessageDao;
        ...
        if is_direct {
            ...
            if let Some(mut existing) = channel_dao.get_direct_channel_by_id(message.channel_id)? {
                ...
                if let Err(e) = channel_dao.upsert(&existing) {
                    warn!("接收消息后更新私聊会话失败: channel_id={}, error={:?}", message.channel_id, e);
                } else {
                    debug!("接收消息后已更新私聊会话: channel_id={}", message.channel_id);
                }
            } else {
                ...
                if let Err(e) = channel_dao.upsert(&new_channel) {
                    warn!("接收消息后创建私聊会话失败: channel_id={}, error={:?}", message.channel_id, e);
                } else {
                    info!("接收消息后已创建私聊会话: channel_id={}", message.channel_id);
                }
            }
        } else if channel_dao.get_by_channel(message.channel_id, message.channel_type)?.is_none() {
            let now = chrono::Utc::now().timestamp_millis();
            let new_channel = entities::Channel {
                id: None,
                channel_id: message.channel_id,
            ...
            if let Err(e) = channel_dao.upsert(&new_channel) {
                warn!("接收消息后创建会话失败: channel_id={}, error={:?}", message.channel_id, e);
            } else {
                info!("接收消息后已创建/确保会话: channel_id={}, channel_type={}", message.channel_id, message.channel_type);
            }
        }
    ...
    /// 处理：查询
    fn handle_query(&mut self, uid: &str, sql: &str, params: &[rusqlite::types::Value]) -> Result<Vec<serde_json::Value>> {
        let conn = self.connections.get(uid)
            .ok_or_else(|| PrivchatSDKError::Database(format!("用户数据库不存在: {}", uid)))?;
        ...
        for row in rows {
            results.push(row.map_err(|e| PrivchatSDKError::Database(format!("读取行失败: {}", e)))?);
        }
    ...
    fn handle_get_messages_before(
        ...
        debug!("[Rust SDK] 📖 从本地 DB 读取: channel_id={}, before_id={}, limit={}, 返回 {} 条", channel_id, before_id, limit, messages.len());
        Ok(messages)
    }
    
    /// 处理：获取频道当前最小的 message.id（用于「加载更早」分页游标）
    fn handle_get_earliest_id(
        &mut self,
        uid: &str,
        channel_id: u64,
    ) -> Result<Option<u64>> {
        ...
        let min_id: Option<i64> = conn.query_row(
            sql,
    ...
    fn handle_update_message_content(&mut self, uid: &str, id: i64, new_content: &str) -> Result<()> {
        let conn = self.connections.get(uid)
            .ok_or_else(|| PrivchatSDKError::Database(format!("用户数据库不存在: {}", uid)))?;
    ...
    fn handle_add_message_reaction(&mut self, uid: &str, id: i64, user_id: u64, reaction: &str) -> Result<()> {
        let conn = self.connections.get(uid)
            .ok_or_else(|| PrivchatSDKError::Database(format!("用户数据库不存在: {}", uid)))?;
    ...
    fn handle_get_channels(&mut self, uid: &str) -> Result<Vec<entities::Channel>> {
        ...
        for row in rows {
            raw.push(row.map_err(|e| PrivchatSDKError::Database(format!("解析行失败: {}", e)))?);
        }
        
        // 私聊（channel_type 0 与 1）按 channel_id 去重，只保留一条，避免同一会话出现两条
        let mut direct_by_id: std::collections::HashMap<u64, entities::Channel> = std::collections::HashMap::new();
        ...
        for ch in raw {
            if ch.channel_type == 0 || ch.channel_type == 1 {
                if let Some(existing) = direct_by_id.get_mut(&ch.channel_id) {
                    ...
                    if !ch.channel_name.is_empty() { existing.channel_name = ch.channel_name.clone(); }
                    if !ch.avatar.is_empty() { existing.avatar = ch.avatar.clone(); }
    ...
    /// 处理：根据频道获取会话
    fn handle_get_channel_by_channel(&mut self, uid: &str, channel_id: u64, channel_type: u8) -> Result<Option<entities::Channel>> {
        let conn = self.connections.get(uid)
            .ok_or_else(|| PrivchatSDKError::Database(format!("用户数据库不存在: {}", uid)))?;
    ...
    fn handle_get_direct_channel_by_id(&mut self, uid: &str, channel_id: u64) -> Result<Option<entities::Channel>> {
        let conn = self.connections.get(uid)
            .ok_or_else(|| PrivchatSDKError::Database(format!("用户数据库不存在: {}", uid)))?;
    ...
    /// 处理：更新会话的 pts
    fn handle_update_channel_pts(&mut self, uid: &str, channel_id: u64, channel_type: u8, new_pts: u64) -> Result<()> {
        let conn = self.connections.get(uid)
            .ok_or_else(|| PrivchatSDKError::Database(format!("用户数据库不存在: {}", uid)))?;
    ...
    fn handle_find_channel_id_by_user(&mut self, uid: &str, target_user_id: u64) -> Result<Option<u64>> {
        let conn = self.connections.get(uid)
            .ok_or_else(|| PrivchatSDKError::Database(format!("用户数据库不存在: {}", uid)))?;
    ...
    fn handle_update_channel_save(&mut self, uid: &str, channel_id: u64, channel_type: i32, save: i32) -> Result<()> {
        let conn = self.connections.get(uid)
            .ok_or_else(|| PrivchatSDKError::Database(format!("用户数据库不存在: {}", uid)))?;
    ...
    fn handle_update_channel_mute(&mut self, uid: &str, channel_id: u64, channel_type: i32, mute: i32) -> Result<()> {
        let conn = self.connections.get(uid)
            .ok_or_else(|| PrivchatSDKError::Database(format!("用户数据库不存在: {}", uid)))?;
    ...
    /// 处理：更新会话的 extra 字段
    fn handle_update_channel_extra(&mut self, uid: &str, channel_id: u64, channel_type: i32, extra: &str) -> Result<()> {
        let conn = self.connections.get(uid)
            .ok_or_else(|| PrivchatSDKError::Database(format!("用户数据库不存在: {}", uid)))?;
    ...
    fn handle_save_friend(&mut self, uid: &str, friend: &entities::Friend) -> Result<()> {
        ...
        fd.upsert(friend).map_err(|e| PrivchatSDKError::Database(format!("保存好友失败: {}", e)))?;
        debug!("✅ [DbActor] 保存好友成功: uid={}, user_id={}", uid, friend.user_id);
        Ok(())
    }
    ...
    fn handle_save_friends(&mut self, uid: &str, friends: &[entities::Friend]) -> Result<()> {
        let conn = self.connections.get(uid)
            .ok_or_else(|| PrivchatSDKError::Database(format!("用户数据库不存在: {}", uid)))?;
        let fd = dao::FriendDao::new(conn);
        fd.upsert_many(friends).map_err(|e| PrivchatSDKError::Database(format!("批量保存好友失败: {}", e)))?;
        info!("✅ [DbActor] 批量保存好友成功: uid={}, count={}", uid, friends.len());
        Ok(())
    }

    /// 处理：获取好友列表（分页）
    fn handle_get_friends(&self, uid: &str, limit: u32, offset: u32) -> Result<Vec<entities::Friend>> {
        let conn = self.connections.get(uid)
            .ok_or_else(|| PrivchatSDKError::Database(format!("用户数据库不存在: {}", uid)))?;
        let fd = dao::FriendDao::new(conn);
        let friends = fd.list(limit, offset).map_err(|e| PrivchatSDKError::Database(format!("查询好友失败: {}", e)))?;
        debug!("✅ [DbActor] 查询好友成功: uid={}, count={}", uid, friends.len());
        Ok(friends)
    }
    ...
    fn handle_get_friends_count(&self, uid: &str) -> Result<u32> {
        let conn = self.connections.get(uid)
            .ok_or_else(|| PrivchatSDKError::Database(format!("用户数据库不存在: {}", uid)))?;
        let fd = dao::FriendDao::new(conn);
        let count = fd.count().map_err(|e| PrivchatSDKError::Database(format!("查询好友总数失败: {}", e)))?;
        debug!("✅ [DbActor] 查询好友总数成功: uid={}, count={}", uid, count);
        Ok(count)
    }
    ...
    fn handle_delete_friend(&mut self, uid: &str, user_id: u64) -> Result<()> {
        let conn = self.connections.get(uid)
            .ok_or_else(|| PrivchatSDKError::Database(format!("用户数据库不存在: {}", uid)))?;
        let fd = dao::FriendDao::new(conn);
        fd.delete_by_user_id(user_id).map_err(|e| PrivchatSDKError::Database(format!("删除好友失败: {}", e)))?;
        info!("✅ [DbActor] 删除好友成功: uid={}, user_id={}", uid, user_id);
        Ok(())
    }
    ...
    fn handle_save_user(&mut self, uid: &str, user: &entities::User) -> Result<()> {
        ...
        ud.upsert(user).map_err(|e| PrivchatSDKError::Database(format!("保存用户失败: {}", e)))?;
        debug!("✅ [DbActor] 保存用户成功: uid={}, user_id={}", uid, user.user_id);
        Ok(())
    }
    ...
    fn handle_save_users(&mut self, uid: &str, users: &[entities::User]) -> Result<()> {
        ...
        let ud = dao::UserDao::new(conn);
        for u in users {
            ud.upsert(u).map_err(|e| PrivchatSDKError::Database(format!("批量保存用户失败: {}", e)))?;
        }
        info!("✅ [DbActor] 批量保存用户成功: uid={}, count={}", uid, users.len());
        Ok(())
    }
    ...
    fn handle_get_user(&self, uid: &str, user_id: u64) -> Result<Option<entities::User>> {
        let conn = self.connections.get(uid)
            .ok_or_else(|| PrivchatSDKError::Database(format!("用户数据库不存在: {}", uid)))?;
        let ud = dao::UserDao::new(conn);
        ud.get_by_id(user_id).map_err(|e| PrivchatSDKError::Database(format!("查询用户失败: {}", e)))
    }
    ...
    fn handle_get_users_by_ids(&self, uid: &str, ids: &[u64]) -> Result<Vec<entities::User>> {
        let conn = self.connections.get(uid)
            .ok_or_else(|| PrivchatSDKError::Database(format!("用户数据库不存在: {}", uid)))?;
        let ud = dao::UserDao::new(conn);
        ud.get_by_ids(ids).map_err(|e| PrivchatSDKError::Database(format!("批量查询用户失败: {}", e)))
    }
    
    /// 处理：保存频道成员
    fn handle_save_channel_member(&mut self, uid: &str, member: &entities::ChannelMember) -> Result<()> {
        let conn = self.connections.get(uid)
            .ok_or_else(|| PrivchatSDKError::Database(format!("用户数据库不存在: {}", uid)))?;
        ...
        debug!("✅ [DbActor] 保存频道成员成功: uid={}, channel_id={}, member_uid={}", 
               uid, member.channel_id, member.member_uid);
        Ok(())
    }
    
    /// 处理：批量保存频道成员
    fn handle_save_channel_members(&mut self, uid: &str, members: &Vec<entities::ChannelMember>) -> Result<()> {
        let conn = self.connections.get(uid)
            .ok_or_else(|| PrivchatSDKError::Database(format!("用户数据库不存在: {}", uid)))?;
        ...
        info!("✅ [DbActor] 批量保存频道成员成功: uid={}, count={}", uid, members.len());
        Ok(())
    }
    ...
    fn handle_get_group_members(
        ...
        let dao = dao::ChannelMemberDao::new(conn);
        let members = dao.list_members(group_id, 2, limit, offset)
            .map_err(|e| PrivchatSDKError::Database(format!("查询群成员失败: {}", e)))?;
        
        debug!("✅ [DbActor] 查询群成员成功: uid={}, group_id={}, count={}", 
               uid, group_id, members.len());
        Ok(members)
    }
    
    fn handle_get_groups(&self, uid: &str, limit: u32, offset: u32) -> Result<Vec<entities::Group>> {
        let conn = self.connections.get(uid)
            .ok_or_else(|| PrivchatSDKError::Database(format!("用户数据库不存在: {}", uid)))?;
        let dao = dao::GroupDao::new(conn);
        let list = dao.list(limit, offset)
            .map_err(|e| PrivchatSDKError::Database(format!("查询群列表失败: {}", e)))?;
        debug!("✅ [DbActor] 查询群列表成功: uid={}, count={}", uid, list.len());
        Ok(list)
    }
    ...
    fn handle_save_groups(&mut self, uid: &str, groups: &[entities::Group]) -> Result<()> {
        ...
        let dao = dao::GroupDao::new(conn);
        for g in groups {
            dao.upsert(g).map_err(|e| PrivchatSDKError::Database(format!("保存群失败: {}", e)))?;
        }
        info!("✅ [DbActor] 批量保存群成功: uid={}, count={}", uid, groups.len());
        Ok(())
    }
    ...
    fn handle_close_user(&mut self, uid: &str) -> Result<()> {
        if let Some(conn) = self.connections.remove(uid) {
            drop(conn);
            info!("✅ [Thread {:?}] 已关闭用户数据库: uid={}", self.thread_id, uid);
        }
        Ok(())
    }
}
...
impl DbActorHandle {
    ...
    pub async fn init_user(&self, uid: String, db_path: PathBuf) -> Result<()> {
        ...
        self.sender.send(DbCommand::InitUser {
            uid,
            db_path,
            respond_to: tx,
        }).map_err(|_| PrivchatSDKError::Other("DB Actor 已停止".to_string()))?;
        
        rx.await.map_err(|_| PrivchatSDKError::Other("DB Actor 响应失败".to_string()))?
    }
    
    /// 更新消息状态（按 message.id）
    pub async fn update_message_status(&self, uid: String, id: i64, status: i32) -> Result<()> {
        tracing::debug!("📤 [DbActorHandle] 发送命令: UpdateMessageStatus(uid={}, id={}, status={})", uid, id, status);
        
        let (tx, rx) = tokio::sync::oneshot::channel();
        
        self.sender.send(DbCommand::UpdateMessageStatus {
            uid: uid.clone(),
            id,
            status,
            respond_to: tx,
    ...
    /// 更新消息的服务端 ID（按 message.id，仅协议层写入）
    pub async fn update_message_server_id(&self, uid: String, id: i64, server_message_id: u64) -> Result<()> {
        let (tx, rx) = tokio::sync::oneshot::channel();
        
        self.sender.send(DbCommand::UpdateMessageServerId {
            uid,
            id,
            server_message_id,
            respond_to: tx,
    ...
    /// 保存接收的消息
    pub async fn save_received_message(&self, uid: String, message: Message) -> Result<i64> {
        let (tx, rx) = tokio::sync::oneshot::channel();
        
        self.sender.send(DbCommand::SaveReceivedMessage {
            uid,
            message,
            respond_to: tx,
        }).map_err(|_| PrivchatSDKError::Other("DB Actor 已停止".to_string()))?;
    ...
    /// 执行 SQL
    pub async fn execute(&self, uid: String, sql: String, params: Vec<rusqlite::types::Value>) -> Result<usize> {
        let (tx, rx) = tokio::sync::oneshot::channel();
    ...
    /// 查询
    pub async fn query(&self, uid: String, sql: String, params: Vec<rusqlite::types::Value>) -> Result<Vec<serde_json::Value>> {
        let (tx, rx) = tokio::sync::oneshot::channel();
    ...
    pub async fn get_message_by_id(&self, uid: String, id: i64) -> Result<Option<Message>> {
        ...
        self.sender.send(DbCommand::GetMessageById {
            uid,
            id,
            respond_to: tx,
        }).map_err(|_| PrivchatSDKError::Other("DB Actor 已停止".to_string()))?;
    ...
    pub async fn get_message_channel_id(&self, uid: String, id: i64) -> Result<Option<u64>> {
        ...
        self.sender.send(DbCommand::GetMessageChannelId {
            uid,
            id,
            respond_to: tx,
        }).map_err(|_| PrivchatSDKError::Other("DB Actor 已停止".to_string()))?;
    ...
    pub async fn get_messages_before(
        ...
        self.sender.send(DbCommand::GetMessagesBefore {
            uid,
            channel_id,
            before_id,
            limit,
    ...
    pub async fn get_earliest_id(
        ...
        self.sender.send(DbCommand::GetEarliestId {
            uid,
            channel_id,
            respond_to: tx,
        }).map_err(|_| PrivchatSDKError::Other("DB Actor 已停止".to_string()))?;
    ...
    pub async fn get_messages_after(
        ...
        self.sender.send(DbCommand::GetMessagesAfter {
            uid,
            channel_id,
            after_id,
            limit,
    ...
    pub async fn send_message(
        ...
        self.sender.send(DbCommand::SendMessage {
            uid,
            channel_id,
            channel_type,
            from_uid,
    ...
    pub async fn revoke_message_with_revoker(&self, uid: String, id: i64, revoker_id: u64) -> Result<()> {
        let (tx, rx) = tokio::sync::oneshot::channel();
        self.sender.send(DbCommand::RevokeMessage {
            uid: uid.clone(),
            id,
            revoker_id,
            respond_to: tx,
    ...
    pub async fn edit_message(&self, uid: String, id: i64, new_content: String) -> Result<()> {
        let (tx, rx) = tokio::sync::oneshot::channel();
        self.sender.send(DbCommand::EditMessage {
            uid,
            id,
            new_content,
            respond_to: tx,
    ...
    pub async fn delete_message(&self, uid: String, id: i64) -> Result<()> {
        ...
        self.sender.send(DbCommand::DeleteMessage {
            uid,
            id,
            respond_to: tx,
        }).map_err(|_| PrivchatSDKError::Other("DB Actor 已停止".to_string()))?;
    ...
    /// 更新消息内容（按 message.id）
    pub async fn update_message_content(&self, uid: String, id: i64, new_content: String) -> Result<()> {
        let (tx, rx) = tokio::sync::oneshot::channel();
        
        self.sender.send(DbCommand::UpdateMessageContent {
            uid,
            id,
            new_content,
            respond_to: tx,
    ...
    /// 添加消息反应（按 message.id）
    pub async fn add_message_reaction(&self, uid: String, id: i64, user_id: u64, reaction: String) -> Result<()> {
        let (tx, rx) = tokio::sync::oneshot::channel();
        
        self.sender.send(DbCommand::AddMessageReaction {
            uid,
            id,
            user_id,
            reaction,
    ...
    pub async fn save_channel(&self, uid: String, channel: entities::Channel) -> Result<()> {
        ...
        self.sender.send(DbCommand::SaveChannel {
            uid,
            channel,
            respond_to: tx,
        }).map_err(|_| PrivchatSDKError::Other("DB Actor 已停止".to_string()))?;
    ...
    pub async fn get_channels(&self, uid: String) -> Result<Vec<entities::Channel>> {
        ...
        self.sender.send(DbCommand::GetChannels {
            uid,
            respond_to: tx,
        }).map_err(|_| PrivchatSDKError::Other("DB Actor 已停止".to_string()))?;
    ...
    /// 根据频道获取会话
    pub async fn get_channel_by_channel(&self, uid: String, channel_id: u64, channel_type: u8) -> Result<Option<entities::Channel>> {
        let (tx, rx) = tokio::sync::oneshot::channel();
        
        self.sender.send(DbCommand::GetChannelByChannel {
            uid,
            channel_id,
            channel_type,
            respond_to: tx,
    ...
    pub async fn get_direct_channel_by_id(&self, uid: String, channel_id: u64) -> Result<Option<entities::Channel>> {
        let (tx, rx) = tokio::sync::oneshot::channel();
        
        self.sender.send(DbCommand::GetDirectChannelById {
            uid,
            channel_id,
            respond_to: tx,
        }).map_err(|_| PrivchatSDKError::Other("DB Actor 已停止".to_string()))?;
    ...
    /// 更新会话的 pts
    pub async fn update_channel_pts(&self, uid: String, channel_id: u64, channel_type: u8, new_pts: u64) -> Result<()> {
        let (tx, rx) = tokio::sync::oneshot::channel();
        
        self.sender.send(DbCommand::UpdateChannelPts {
            uid,
            channel_id,
            channel_type,
            new_pts,
    ...
    /// 根据用户查找 channel_id
    pub async fn find_channel_id_by_user(&self, uid: String, target_user_id: u64) -> Result<Option<u64>> {
        let (tx, rx) = tokio::sync::oneshot::channel();
        
        self.sender.send(DbCommand::FindChannelIdByUser {
            uid,
            target_user_id,
            respond_to: tx,
        }).map_err(|_| PrivchatSDKError::Other("DB Actor 已停止".to_string()))?;
    ...
    /// 更新频道的 save 字段（收藏状态）
    pub async fn update_channel_save(&self, uid: String, channel_id: u64, channel_type: u8, save: i32) -> Result<()> {
        let (tx, rx) = tokio::sync::oneshot::channel();
        
        self.sender.send(DbCommand::UpdateChannelSave {
            uid,
            channel_id,
    ...
    /// 更新频道的 mute 字段（通知模式）
    pub async fn update_channel_mute(&self, uid: String, channel_id: u64, channel_type: u8, mute: i32) -> Result<()> {
        let (tx, rx) = tokio::sync::oneshot::channel();
        
        self.sender.send(DbCommand::UpdateChannelMute {
            uid,
            channel_id,
            channel_type: channel_type as i32,
            mute,
    ...
    /// 更新会话的 extra 字段
    pub async fn update_channel_extra(&self, uid: String, channel_id: u64, channel_type: u8, extra: String) -> Result<()> {
        let (tx, rx) = tokio::sync::oneshot::channel();
        
        self.sender.send(DbCommand::UpdateChannelExtra {
            uid,
            channel_id,
            channel_type: channel_type as i32,
            extra,
    ...
    pub async fn save_user(&self, uid: String, user: entities::User) -> Result<()> {
        let (tx, rx) = tokio::sync::oneshot::channel();
        self.sender.send(DbCommand::SaveUser { uid, user, respond_to: tx })
            .map_err(|_| PrivchatSDKError::Other("DB Actor 已停止".to_string()))?;
        rx.await.map_err(|_| PrivchatSDKError::Other("DB Actor 响应失败".to_string()))?
    }
    ...
    pub async fn save_users(&self, uid: String, users: Vec<entities::User>) -> Result<()> {
        let (tx, rx) = tokio::sync::oneshot::channel();
        self.sender.send(DbCommand::SaveUsers { uid, users, respond_to: tx })
            .map_err(|_| PrivchatSDKError::Other("DB Actor 已停止".to_string()))?;
        rx.await.map_err(|_| PrivchatSDKError::Other("DB Actor 响应失败".to_string()))?
    }
    ...
    pub async fn get_user(&self, uid: String, user_id: u64) -> Result<Option<entities::User>> {
        let (tx, rx) = tokio::sync::oneshot::channel();
        self.sender.send(DbCommand::GetUser { uid, user_id, respond_to: tx })
            .map_err(|_| PrivchatSDKError::Other("DB Actor 已停止".to_string()))?;
        rx.await.map_err(|_| PrivchatSDKError::Other("DB Actor 响应失败".to_string()))?
    }

    pub async fn get_users_by_ids(&self, uid: String, ids: Vec<u64>) -> Result<Vec<entities::User>> {
        let (tx, rx) = tokio::sync::oneshot::channel();
        self.sender.send(DbCommand::GetUsersByIds { uid, ids, respond_to: tx })
            .map_err(|_| PrivchatSDKError::Other("DB Actor 已停止".to_string()))?;
        rx.await.map_err(|_| PrivchatSDKError::Other("DB Actor 响应失败".to_string()))?
    }
    ...
    pub async fn save_friend(&self, uid: String, friend: entities::Friend) -> Result<()> {
        ...
        self.sender.send(DbCommand::SaveFriend {
            uid,
            friend,
            respond_to: tx,
        }).map_err(|_| PrivchatSDKError::Other("DB Actor 已停止".to_string()))?;
    ...
    pub async fn save_friends(&self, uid: String, friends: Vec<entities::Friend>) -> Result<()> {
        ...
        self.sender.send(DbCommand::SaveFriends {
            uid,
            friends,
            respond_to: tx,
        }).map_err(|_| PrivchatSDKError::Other("DB Actor 已停止".to_string()))?;
    ...
    /// 从本地数据库获取好友列表（分页）
    pub async fn get_friends(&self, uid: String, limit: u32, offset: u32) -> Result<Vec<entities::Friend>> {
        let (tx, rx) = tokio::sync::oneshot::channel();
        
        self.sender.send(DbCommand::GetFriends {
            uid,
            limit,
            offset,
            respond_to: tx,
    ...
    pub async fn get_friends_count(&self, uid: String) -> Result<u32> {
        ...
        self.sender.send(DbCommand::GetFriendsCount {
            uid,
            respond_to: tx,
        }).map_err(|_| PrivchatSDKError::Other("DB Actor 已停止".to_string()))?;
    ...
    pub async fn delete_friend(&self, uid: String, user_id: u64) -> Result<()> {
        ...
        self.sender.send(DbCommand::DeleteFriend {
            uid,
            user_id,
            respond_to: tx,
        }).map_err(|_| PrivchatSDKError::Other("DB Actor 已停止".to_string()))?;
    ...
    /// 保存频道成员
    pub async fn save_channel_member(&self, uid: String, member: entities::ChannelMember) -> Result<()> {
        let (tx, rx) = tokio::sync::oneshot::channel();
        
        self.sender.send(DbCommand::SaveChannelMember {
            uid,
            member,
            respond_to: tx,
        }).map_err(|_| PrivchatSDKError::Other("DB Actor 已停止".to_string()))?;
    ...
    /// 批量保存频道成员
    pub async fn save_channel_members(&self, uid: String, members: Vec<entities::ChannelMember>) -> Result<()> {
        let (tx, rx) = tokio::sync::oneshot::channel();
        
        self.sender.send(DbCommand::SaveChannelMembers {
            uid,
            members,
            respond_to: tx,
        }).map_err(|_| PrivchatSDKError::Other("DB Actor 已停止".to_string()))?;
    ...
    pub async fn delete_channel_member(
        ...
        let (tx, rx) = tokio::sync::oneshot::channel();
        self.sender.send(DbCommand::DeleteChannelMember {
            uid,
            channel_id,
            channel_type,
            member_uid,
    ...
    pub async fn get_group_members(
        ...
        self.sender.send(DbCommand::GetGroupMembers {
            uid,
            group_id,
            limit,
            offset,
    ...
    /// 获取群列表（分页）
    pub async fn get_groups(&self, uid: String, limit: u32, offset: u32) -> Result<Vec<entities::Group>> {
        let (tx, rx) = tokio::sync::oneshot::channel();
        self.sender.send(DbCommand::GetGroups { uid, limit, offset, respond_to: tx })
            .map_err(|_| PrivchatSDKError::Other("DB Actor 已停止".to_string()))?;
        rx.await.map_err(|_| PrivchatSDKError::Other("DB Actor 响应失败".to_string()))?
    }
    ...
    pub async fn get_group(&self, uid: String, group_id: u64) -> Result<Option<entities::Group>> {
        let (tx, rx) = tokio::sync::oneshot::channel();
        self.sender.send(DbCommand::GetGroup { uid, group_id, respond_to: tx })
            .map_err(|_| PrivchatSDKError::Other("DB Actor 已停止".to_string()))?;
        rx.await.map_err(|_| PrivchatSDKError::Other("DB Actor 响应失败".to_string()))?
    }
    ...
    pub async fn save_groups(&self, uid: String, groups: Vec<entities::Group>) -> Result<()> {
        let (tx, rx) = tokio::sync::oneshot::channel();
        self.sender.send(DbCommand::SaveGroups { uid, groups, respond_to: tx })
            .map_err(|_| PrivchatSDKError::Other("DB Actor 已停止".to_string()))?;
        rx.await.map_err(|_| PrivchatSDKError::Other("DB Actor 响应失败".to_string()))?
    }
    ...
    pub async fn close_user(&self, uid: String) -> Result<()> {
        ...
        self.sender.send(DbCommand::CloseUser {
            uid,
            respond_to: tx,
        }).map_err(|_| PrivchatSDKError::Other("DB Actor 已停止".to_string()))?;
