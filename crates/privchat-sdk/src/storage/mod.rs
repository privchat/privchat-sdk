...
pub use advanced_features::{
    ...
    MessageEditEvent, ChannelReadState
};
...
pub struct StorageManager {
    ...
    user_queue_managers: Arc<RwLock<HashMap<String, Arc<dyn crate::storage::queue::TaskQueueTrait + Send + Sync>>>>,
    /// 媒体索引管理器
...
impl StorageManager {
    ...
    pub async fn new(base_path: &Path, assets_path: Option<&Path>) -> Result<Self> {
        ...
        let db_actor = db_actor::DbActorHandle::spawn(
            assets_path.map(|p| p.to_path_buf())
        );
        tracing::info!("✅ DB Actor 已启动（单线程模型，assets_path={:?}）", assets_path);
    ...
    pub async fn init_user(&self, uid: &str) -> Result<()> {
        ...
        tracing::info!("🔧 正在初始化用户数据库: uid={}, path={}", uid, db_path.display());
    ...
    pub async fn get_all_user_settings(&self) -> Result<std::collections::HashMap<String, serde_json::Value>> {
        ...
        let pairs = kv.scan_prefix::<serde_json::Value>(PREFIX.as_bytes()).await?;
    ...
    pub async fn update_message_status(&self, id: i64, status: i32) -> Result<()> {
        ...
        tracing::debug!("🔍 [StorageManager] update_message_status: uid={}, id={}, status={}", uid, id, status);
        self.db_actor.update_message_status(uid.clone(), id, status).await
            .map_err(|e| { tracing::error!("❌ [StorageManager] update_message_status 失败: uid={}, error={}", uid, e); e })
    }
    ...
    pub async fn update_message_server_id(&self, id: i64, server_message_id: u64) -> Result<()> {
        let uid = self.current_user.read().await.clone()
            .ok_or_else(|| PrivchatSDKError::NotConnected)?;
        self.db_actor.update_message_server_id(uid, id, server_message_id).await
    }
    ...
    pub async fn send_message(
        ...
        self.db_actor.send_message(uid, channel_id, channel_type, from_uid, content.to_string(), message_type).await
    }
    ...
    /// 保存接收到的消息
    pub async fn save_received_message(&self, message: &Message) -> Result<i64> {
        let uid = self.current_user.read().await.clone()
            .ok_or_else(|| PrivchatSDKError::NotConnected)?;
        
        // 通过 DB Actor 保存（单线程安全）
        let row_id = self.db_actor.save_received_message(uid, message.clone()).await?;
        
        // TODO: 更新会话信息（暂时注释，需要重构为 Actor 命令）
        // self.update_channel_after_message(&*conn_guard, message)?;
    ...
    pub async fn save_message(&self, message: &Message) -> Result<i64> {
        let row_id = self.save_received_message(message).await?;
        Ok(row_id)
    }
    ...
    pub async fn revoke_message_by(&self, id: i64, revoker_id: u64) -> Result<()> {
        let uid = self.current_user.read().await.clone()
            .ok_or_else(|| PrivchatSDKError::NotConnected)?;
        self.db_actor.revoke_message_with_revoker(uid, id, revoker_id).await
    }
    ...
    pub async fn update_message_content(&self, id: i64, new_content: &str) -> Result<()> {
        let uid = self.current_user.read().await.clone()
            .ok_or_else(|| PrivchatSDKError::NotConnected)?;
        self.db_actor.update_message_content(uid, id, new_content.to_string()).await
    }
    ...
    pub async fn add_message_reaction(&self, id: i64, user_id: u64, reaction: &str) -> Result<()> {
        let uid = self.current_user.read().await.clone()
            .ok_or_else(|| PrivchatSDKError::NotConnected)?;
        self.db_actor.add_message_reaction(uid, id, user_id, reaction.to_string()).await
    }
    ...
    pub async fn get_messages(&self, _query: &MessageQuery) -> Result<Vec<Message>> {
        Err(PrivchatSDKError::Other(
            "get_messages 已移除直接连接，请使用 get_messages_before 或 get_messages_after".to_string()
        ))
    }
    ...
    pub async fn get_messages_before(
        ...
        let uid = self.current_user.read().await
            .as_ref()
            .ok_or_else(|| PrivchatSDKError::Other("未设置当前用户".to_string()))?
            .clone();
        
        self.db_actor.get_messages_before(uid, channel_id, before_id, limit).await
    }
    ...
    pub async fn get_messages_after(
        ...
        self.db_actor.get_messages_after(uid, channel_id, after_id, limit).await
    }
    ...
    pub async fn get_channel_by_channel(&self, channel_id: u64, channel_type: u8) -> Result<Option<Channel>> {
        let uid = self.current_user.read().await.clone()
            .ok_or_else(|| PrivchatSDKError::NotConnected)?;
        
        // 通过 DB Actor 获取会话
        self.db_actor.get_channel_by_channel(uid, channel_id, channel_type).await
    }
    ...
    pub async fn get_direct_channel_by_id(&self, channel_id: u64) -> Result<Option<Channel>> {
        ...
        self.db_actor.get_direct_channel_by_id(uid, channel_id).await
    }
    
    /// 更新会话的 pts（用于同步）
    pub async fn update_channel_pts(&self, channel_id: u64, channel_type: u8, new_pts: u64) -> Result<()> {
        let uid = self.current_user.read().await.clone()
            .ok_or_else(|| PrivchatSDKError::NotConnected)?;
        
        // 通过 DB Actor 更新 pts
        self.db_actor.update_channel_pts(uid, channel_id, channel_type, new_pts).await
    }
    ...
    pub async fn get_total_unread_count(&self, _exclude_muted: bool) -> Result<i32> {
        Err(PrivchatSDKError::Other(
            "get_total_unread_count 已移除直接连接，请使用 db_actor 或 AdvancedFeaturesIntegration".to_string()
        ))
    }
    
    /// 验证总未读数是否等于会话列表未读数的和（请通过 db_actor 使用）
    pub async fn verify_total_unread_count(&self, _exclude_muted: bool) -> Result<(i32, i32, bool)> {
        Err(PrivchatSDKError::Other(
    ...
    pub async fn get_channel(&self, channel_id: u64, channel_type: i32) -> Result<Option<Channel>> {
        let uid = self.current_user.read().await.clone()
            .ok_or_else(|| PrivchatSDKError::NotConnected)?;
        self.db_actor.get_channel_by_channel(uid, channel_id, channel_type as u8).await
    }
    ...
    {
        Err(PrivchatSDKError::Other(
            "execute_transaction 已移除直接连接，请使用 db_actor.execute 或 db_actor.query".to_string()
        ))
    }
    ...
    fn update_channel_after_message(&self, conn: &Connection, message: &Message) -> Result<()> {
        ...
        // 查找现有会话
        if let Some(mut channel) = channel_dao.get_by_channel(message.channel_id, message.channel_type)? {
            // 更新会话信息
            channel.last_local_message_id = message.local_message_id;
            channel.last_msg_timestamp = message.timestamp;
            channel.last_msg_pts = message.pts;  // ⭐ message_seq -> pts
            channel.version += 1;
            ...
            let new_channel = Channel {
                ...
                unread_count: 0,
                last_msg_pts: message.pts,  // ⭐ message_seq -> pts
                // 频道信息字段（使用默认值）
                show_nick: 0,
                username: String::new(),
    ...
    pub async fn get_friends(
        ...
        let uid = self.current_user.read().await.clone()
            .ok_or_else(|| PrivchatSDKError::Other("用户未初始化".to_string()))?;
        let friends = self.db_actor.get_friends(uid.clone(), limit, offset).await?;
        if friends.is_empty() {
            return Ok(Vec::new());
        }
        ...
        let out: Vec<_> = friends
            .into_iter()
            .map(|f| {
                let u = user_map.get(&f.user_id).cloned().unwrap_or_else(|| entities::User {
                    user_id: f.user_id,
                    username: None,
                    nickname: None,
                    alias: None,
    ...
    pub async fn delete_channel_member(
        ...
        let uid = self.current_user.read().await.clone()
            .ok_or_else(|| PrivchatSDKError::Other("用户未初始化".to_string()))?;
        self.db_actor.delete_channel_member(uid, channel_id, channel_type, member_uid).await
    }
    ...
    pub async fn get_group_members(
        ...
        self.db_actor.get_group_members(uid, group_id, limit, offset).await
    }
    ...
    pub async fn find_channel_id_by_user(&self, target_user_id: u64) -> Result<Option<u64>> {
        ...
        self.db_actor.find_channel_id_by_user(uid, target_user_id).await
    }
    
    /// 更新频道的 save 字段（收藏状态）
    pub async fn update_channel_save(&self, channel_id: u64, channel_type: i32, save: i32) -> Result<()> {
        let uid = self.current_user.read().await.clone()
            .ok_or_else(|| PrivchatSDKError::Other("用户未初始化".to_string()))?;
        
        self.db_actor.update_channel_save(uid, channel_id, channel_type as u8, save).await
    }
    
    /// 更新频道的 mute 字段（通知模式）
    pub async fn update_channel_mute(&self, channel_id: u64, channel_type: i32, mute: i32) -> Result<()> {
        let uid = self.current_user.read().await.clone()
            .ok_or_else(|| PrivchatSDKError::Other("用户未初始化".to_string()))?;
        
        self.db_actor.update_channel_mute(uid, channel_id, channel_type as u8, mute).await
    }
    
    /// 更新会话的 extra 字段
    pub async fn update_channel_extra(&self, channel_id: u64, channel_type: u8, extra: String) -> Result<()> {
        let uid = self.current_user.read().await.clone()
            .ok_or_else(|| PrivchatSDKError::Other("用户未初始化".to_string()))?;
        
        self.db_actor.update_channel_extra(uid, channel_id, channel_type, extra).await
    }
