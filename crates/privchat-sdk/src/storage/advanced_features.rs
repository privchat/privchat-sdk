...
impl AdvancedFeaturesManager {
    ...
    pub fn initialize_tables(&self) -> Result<()> {
        ...
        conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_conversation_read_states_channel 
             ON conversation_read_states(channel_id, channel_type)",
            [],
        )?;
    ...
    /// 更新会话已读状态
    pub fn update_conversation_read_state(
        &self,
        channel_id: &str,
        channel_type: i32,
        user_id: &str,
        ...
        // 更新或插入会话已读状态
        conn.execute(
            "INSERT OR REPLACE INTO conversation_read_states 
        ...
        info!("Updated conversation read state for user {} in channel {}", user_id, channel_id);

        Ok(ConversationReadState {
            channel_id: channel_id.to_string(),
            channel_type,
            user_id: user_id.to_string(),
            last_read_message_id: last_read_message_id.map(|s| s.to_string()),
            last_read_seq,
            last_read_at: now,
            unread_count,
        })
    ...
    /// 获取消息的已读回执列表
    pub fn get_message_read_receipts(&self, message_id: &str) -> Result<Vec<ReadReceiptEvent>> {
        let conn = self.conn.lock().unwrap();
    ...
    /// 获取会话的未读消息数量
    pub fn get_unread_count(&self, channel_id: &str, channel_type: i32, user_id: &str) -> Result<i32> {
        let conn = self.conn.lock().unwrap();
        
        // 获取用户的最后已读序列号
        let last_read_seq: i64 = conn.query_row(
            "SELECT COALESCE(last_read_seq, 0) FROM conversation_read_states 
    ...
    pub fn revoke_message(
        ...
        // 检查消息是否存在并获取消息信息
        let (channel_id, channel_type, from_uid, created_at): (String, i32, String, i64) = conn.query_row(
        ...
        Ok(MessageRevokeEvent {
            ...
            channel_id,
            channel_type,
            revoker_uid: revoker_uid.to_string(),
            revoked_at: now,
    ...
    /// 检查消息是否可以撤回
    pub fn can_revoke_message(&self, message_id: &str, user_id: &str) -> Result<bool> {
        let conn = self.conn.lock().unwrap();

        let result: Option<(String, i32, i64)> = conn.query_row(
    ...
    pub fn edit_message(
        ...
        // 检查消息是否存在并获取消息信息
        let (channel_id, channel_type, from_uid, old_content, created_at): (String, i32, String, String, i64) = conn.query_row(
        ...
        Ok(MessageEditEvent {
            ...
            channel_id,
            channel_type,
            editor_uid: editor_uid.to_string(),
    ...
    /// 获取消息编辑历史
    pub fn get_message_edit_history(&self, message_id: &str) -> Result<Vec<MessageEditEvent>> {
        let conn = self.conn.lock().unwrap();
        let mut stmt = conn.prepare(
    ...
    /// 检查消息是否可以编辑
    pub fn can_edit_message(&self, message_id: &str, user_id: &str) -> Result<bool> {
        let conn = self.conn.lock().unwrap();
    ...
    /// 获取会话的所有已读状态
    pub fn get_conversation_read_states(
        &self,
        channel_id: &str,
        channel_type: i32,
    ) -> Result<Vec<ConversationReadState>> {
        let conn = self.conn.lock().unwrap();
        let mut stmt = conn.prepare(
...
mod tests {
    ...
    #[tokio::test]
    async fn test_conversation_read_state() {
        let (manager, _temp_file) = create_test_manager();
        
        // 测试更新会话已读状态
        let read_state = manager.update_conversation_read_state(
