...
impl<'a> ChannelDao<'a> {
    ...
    pub fn upsert(&self, channel: &Channel) -> Result<i64> {
        let sql = "INSERT OR REPLACE INTO channel (
            ...
            unread_count, is_deleted, version, extra, last_msg_pts,  -- ⭐ last_msg_seq -> last_msg_pts
        ...
        self.conn.execute(sql, params![
            ...
            channel.version,
            channel.extra,
            channel.last_msg_pts,  // ⭐ last_msg_seq -> last_msg_pts
...
mod tests {
    ...
    fn test_get_total_unread_count_exclude_muted() {
        ...
        println!("✅ 排除免打扰测试通过: 排除免打扰={}, 全部={}", total, total_all);
    }
    ...
    fn test_verify_total_unread_count() {
        ...
        println!("✅ 数据一致性验证通过: 总未读数={}, 会话列表和={}", total, sum);
    }
}
