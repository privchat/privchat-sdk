...
impl<'a> MigrationDao<'a> {
    ...
    /// 初始化数据库 - 使用完整的表结构（编译时嵌入）
    pub fn initialize_database(&self) -> Result<()> {
        // 执行完整的数据库初始化脚本
        let sql = include_str!("../../../../../assets/20240101000001.sql");
        
        self.conn.execute_batch(sql)
            .map_err(|e| PrivchatSDKError::Database(format!("数据库初始化失败: {}", e)))?;
    ...
    pub fn get_database_stats(&self) -> Result<DatabaseStats> {
        ...
        let conversation_count: i64 = self.conn.query_row(
            "SELECT COUNT(*) FROM conversation WHERE is_deleted = 0",
            [],
            |row| row.get(0)
        ).unwrap_or(0);
...
mod tests {
    ...
    fn test_migration_initialize() {
        ...
        // 初始化数据库
        migration.initialize_database().unwrap();
        
        // 验证版本
        let version = migration.get_current_version().unwrap();
        assert_eq!(version, Some("20240101000001".to_string()));
        
        // 验证表结构
        assert!(migration.validate_schema().unwrap());
    }
