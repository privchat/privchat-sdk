...
impl<'a> ChannelDao<'a> {
    ...
    pub fn find_by_username(&self, username: &str) -> Result<Option<Channel>> {
        ...
        let mut stmt = self.conn.prepare(sql)?;
        let mut rows = stmt.query_map(params![username], |row| Ok(self.row_to_channel(row)?))?;
    ...
    pub fn upsert(&self, channel: &Channel) -> Result<()> {
        let sql = r#"
            INSERT INTO channel (
                channel_id, channel_type, last_local_message_id, last_msg_timestamp, last_msg_content,
                unread_count, last_msg_pts, show_nick, username, channel_name, channel_remark,
        ...
        self.conn.execute(
            ...
            params![
                ...
                channel.remote_extra,
                channel.flame,
                channel.flame_second,
                channel.device_flag,
