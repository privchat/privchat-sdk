...
impl TestPhases {
    ...
    ///
    /// alice 发给 bob，bob 把**同一份内容**再发一次。bob 手上是自己下载下来的
    /// 那份文件，服务端已经存过它，所以：
    ///
    /// - **主文件**零字节上传：bob 拿到属于自己的 file_id，claim 命中（`claims` +1）。
    /// - **缩略图照传一次**：它是本地按原图重新生成的派生物，字节每次都不同，
    ///   永远命中不了秒传。这是已知代价，单独计数免得混进「主文件省了没」。
    /// - 把 bob 的封装缓存删掉再发一次：密文变了、服务端没见过，必须退回整传，
    ///   而且**照样发得出去**（这条链断了的话，重发会永久卡住）。
    /// - 全程走普通发送接口：没有 forward RPC，也没有第二条上传路径。
    ///
    /// 🔴 这条**不是** App 转发功能的验收：它驱动的是 Rust SDK 的发送路径，没有经过
    /// App 的 `planResend` / Kotlin `sendMedia`，也没有断言接收方那边渲染成什么样。
    /// 重发进的是 alice↔bob 这条会话（bob 直发陌生人 charlie 会被服务端按陌生人策略
    /// 拒掉，那是另一条规则）——收件人是谁不影响这里要证的事。
