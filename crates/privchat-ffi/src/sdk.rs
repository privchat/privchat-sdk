...
use crate::{
    ...
    error::PrivchatError,
    events::{
        AttachmentInfo, AttachmentSendResult, AuthResult, BlacklistEntry, ChannelListEntry,
        ChannelListObserver, ChannelTags, ConnectionState, DeviceSummary, FriendEntry,
        FriendPendingEntry, GetOrCreateDirectChannelResult, GroupCreateResult, GroupEntry,
        GroupMemberEntry, GroupQRCodeJoinResult, LastReadPosition, LatestChannelEvent,
        MediaProcessOp, MessageEntry, NotificationMode, PresenceEntry, PrivchatDelegate,
        ProgressObserver, ReactionChip, ReadReceiptEvent, ReceiptsObserver, SDKEvent, SearchHit,
        SearchPage, SeenByEntry, SendMessageOptions, SendObserver, SendState, SendUpdate,
        SyncObserver, SyncStateEntry, TimelineDiffKind, TimelineObserver, TypingIndicatorEvent,
        TypingObserver, UnreadStats, UserEntry, VideoProcessHook,
    },
    helpers::get_runtime,
};
...
/// Registry for SDK instances by stable id; avoids using Kotlin-passed pointer as Arc on iOS.
static SDK_REGISTRY: OnceLock<std::sync::Mutex<HashMap<u64, Arc<PrivchatSDK>>>> = OnceLock::new();
/// 第一个实例 id 为 0，与 Kotlin 侧 parseU64FromResultBuffer 及 login_by_handle 一致
static SDK_NEXT_ID: AtomicU64 = AtomicU64::new(0);

fn sdk_registry() -> &'static std::sync::Mutex<HashMap<u64, Arc<PrivchatSDK>>> {
    SDK_REGISTRY.get_or_init(|| std::sync::Mutex::new(HashMap::new()))
}

/// Register the UniFFI pointer as a stable id; call once after constructor from Kotlin.
/// Returns the id to use with login_by_handle; never use the pointer for from_raw in login.
#[uniffi::export]
pub fn register_sdk_handle(pointer_handle: u64) -> Result<u64, PrivchatError> {
    eprintln!(
        "[FFI] register_sdk_handle entry pointer_handle={}",
        pointer_handle
    );
    info!(
        "[FFI] register_sdk_handle entry pointer_handle={}",
    ...
    if pointer_handle == 0 {
        eprintln!("[FFI] register_sdk_handle rejected null handle");
        warn!("[FFI] register_sdk_handle rejected null handle");
        return Err(PrivchatError::NotInitialized);
    }
    unsafe { std::sync::Arc::increment_strong_count(pointer_handle as *const PrivchatSDK) };
    let arc = unsafe { std::sync::Arc::from_raw(pointer_handle as *const PrivchatSDK) };
    let id = SDK_NEXT_ID.fetch_add(1, Ordering::Relaxed);
    sdk_registry()
        .lock()
        .map_err(|_| PrivchatError::Generic {
            msg: "registry lock".into(),
        })?
        .insert(id, arc);
    eprintln!(
        "[FFI] register_sdk_handle ok id={} (use this id in login_by_handle)",
        id
    );
    info!(
        "[FFI] register_sdk_handle ok id={} (use this id in login_by_handle)",
...
/// Unregister and drop the SDK ref; call from Kotlin close() before free(pointer_handle).
/// id 0 是合法值（第一个实例），必须从 registry 移除，否则会泄漏。
#[uniffi::export]
pub fn unregister_sdk_handle(id: u64) -> Result<(), PrivchatError> {
    let _ = sdk_registry()
        .lock()
        .map_err(|_| PrivchatError::Generic {
            msg: "registry lock".into(),
...
impl PrivchatSDK {
    ...
    ///
    /// Runs in a dedicated Rust thread via block_on so we never block_on from the caller
    /// (e.g. Kotlin/GCD thread), avoiding deadlock with Tokio RwLock/semaphore.
    pub fn connect(self: Arc<Self>) -> Result<(), PrivchatError> {
        // 注意：iOS 上 eprintln! 通常不会出现在 Xcode 控制台（dylib 的 stderr 未连到调试器），
        // 关键节点请以 Kotlin 侧 [SDK.iOS] 日志为准。
        eprintln!("[FFI] connect() entry");
        sdk_ffi_log!("connect");
        ...
        std::thread::spawn(move || {
            ...
            eprintln!("[FFI] connect() worker thread, calling connect_async");
            let _ = tx.send(get_runtime().block_on(sdk.connect_async()));
        });
        eprintln!("[FFI] connect() waiting recv");
        let r = rx.recv().map_err(|_| PrivchatError::Generic {
            msg: "connect: channel recv".into(),
        })?;
        eprintln!("[FFI] connect() recv done: {}", if r.is_ok() { "Ok" } else { "Err" });
        r
    }

    async fn connect_async(self: Arc<Self>) -> Result<(), PrivchatError> {
        eprintln!("[FFI] connect_async() start");
        ...
        // 2. 启动事件监听器（如果还没有启动）
        {
            ...
            if let Some(sdk) = guard.as_ref() {
                ...
                // Clone so we can release self.inner read lock before long-running awaits (avoids deadlock:
                // holding the lock across await blocks other tasks e.g. event listener that need the lock).
                let sdk = sdk.clone();
                drop(guard);

                // 3. 调用真实的连接逻辑（不再持有 self.inner，避免死锁）
                eprintln!("[FFI] connect_async() calling sdk.connect().await");
                sdk.connect().await?;
                eprintln!("[FFI] connect_async() sdk.connect() ok");
                ...
                eprintln!("[FFI] connect_async() inner is None");
    ...
    /// Runs in a dedicated Rust thread via block_on so we never block_on from the caller thread.
    pub fn disconnect(self: Arc<Self>) -> Result<(), PrivchatError> {
        sdk_ffi_log!("disconnect");
        let (tx, rx) = std::sync::mpsc::sync_channel(0);
        let sdk = self.clone();
        std::thread::spawn(move || {
            let _ = tx.send(get_runtime().block_on(sdk.disconnect_async()));
        });
        rx.recv().map_err(|_| PrivchatError::Generic {
            msg: "disconnect: channel recv".into(),
        })?
    }

    async fn disconnect_async(self: Arc<Self>) -> Result<(), PrivchatError> {
        let guard = self.inner.read().await;
        if let Some(sdk) = guard.as_ref() {
    ...
    pub fn send_message_with_options(
        ...
        get_runtime().block_on(async move {
            ...
            if let Some(sdk) = guard.as_ref() {
                ...
                let id = sdk
                    .send_message_with_options(channel_id, &content, sdk_options)
                    .await?;
    ...
    pub fn mark_as_read(
        ...
        debug!(
            "Marking message {} as read in channel {}",
    ...
    pub fn send_attachment_from_path(
        ...
        debug!(
            "Sending attachment from path: channel_id={}, file_path={}",
        ...
        get_runtime().block_on(async move {
            ...
            if let Some(sdk) = guard.as_ref() {
                ...
                // 调用 SDK 层方法
                let (id, sdk_attachment) = sdk
                    .send_attachment_from_path(channel_id, &path, sdk_options, progress_callback)
                    .await?;
                ...
                info!(
                    "✅ Attachment sent successfully, id: {}, file_id: {:?}",
                    id, ffi_attachment.file_id
                );
    ...
    pub fn send_attachment_bytes(
        ...
        debug!(
            "Sending attachment from bytes: channel_id={}, filename={}, size={}",
            channel_id,
            filename,
            data.len()
        );
    ...
    pub fn download_attachment_to_cache(
        ...
        debug!(
            "Downloading attachment to cache: file_id={}, file_url={}",
            file_id, file_url
        );
        ...
        get_runtime().block_on(async move {
            ...
            if let Some(sdk) = guard.as_ref() {
                let cache_path = sdk
                    .download_attachment_to_cache(&file_id, &file_url, progress_callback)
                    .await?;
    ...
    pub fn download_attachment_to_path(
        ...
        debug!(
            "Downloading attachment to path: file_url={}, output_path={}",
        ...
        get_runtime().block_on(async move {
            let guard = inner.read().await;
            if let Some(sdk) = guard.as_ref() {
                sdk.download_attachment_to_path(&file_url, &path, progress_callback)
                    .await?;
        ...
        get_runtime().block_on(async move {
            ...
            if let Some(sdk) = guard.as_ref() {
                ...
                state
                    .user
                    .as_ref()
                    .and_then(|info| info.user_id.parse::<u64>().ok())
            } else {
                None
            }
        })
            ...
            if let Some(sdk) = guard.take() {
                match sdk.shutdown().await {
                    ...
                    Err(e) => {
                        ...
                        Err(PrivchatError::Generic {
                            msg: "SDK shutdown failed".to_string(),
                        })
                    }
                }
            } else {
            ...
            let sdk_hook = hook.map(|h| {
                ...
                let closure: privchat_sdk::storage::media_preprocess::VideoProcessHook =
                    std::sync::Arc::new(move |op, p1, p2, p3| {
                        let op_ffi = match op {
                            privchat_sdk::storage::media_preprocess::MediaProcessOp::Thumbnail => {
                                MediaProcessOp::Thumbnail
                            }
                            privchat_sdk::storage::media_preprocess::MediaProcessOp::Compress => {
    ...
    ///
    /// Sync FFI: Kotlin/iOS calls sync C API. catch_unwind + recv_timeout to avoid process crash on worker panic.
    pub fn register(
        &self,
        username: String,
        password: String,
        ...
        std::thread::spawn(move || {
            let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
                get_runtime().block_on(async move {
                    ...
                    if let Some(sdk) = guard.as_ref() {
                        ...
                        let (user_id, token) = sdk
                            .register(username, password, device_id, Some(device_info))
                            .await?;
                        Ok(AuthResult { user_id, token })
                    } else {
                        Err(PrivchatError::NotInitialized)
                    }
                })
            ...
            let _ = tx.send(match result {
                Ok(Ok(v)) => Ok(v),
                Ok(Err(e)) => Err(e),
                Err(_) => Err(PrivchatError::Generic {
                    msg: "register: worker panic".into(),
                }),
            });
        });
        ...
        rx.recv_timeout(std::time::Duration::from_secs(60))
            .map_err(|e| match e {
                RecvTimeoutError::Timeout => PrivchatError::Generic {
                    msg: "register: timeout".into(),
                },
                RecvTimeoutError::Disconnected => PrivchatError::Generic {
                    msg: "register: channel disconnected".into(),
                },
            })?
    }

    /// Sync FFI: Kotlin/iOS should call [login_by_handle] instead to avoid bad &self on iOS.
    /// This method remains for Swift/other bindings that resolve the handle correctly.
    pub fn login(
        &self,
        username: String,
        password: String,
        ...
        match result {
            ...
            Ok(Err(e)) => Err(e),
            Err(_) => Err(PrivchatError::Generic {
                msg: "login: panic".into(),
            }),
        }
    }

    /// Sync FFI: catch_unwind + recv_timeout to avoid process crash on worker panic.
    pub fn authenticate(
        &self,
        user_id: u64,
        token: String,
        ...
        std::thread::spawn(move || {
            ...
            let _ = tx.send(match result {
                ...
                Ok(Err(e)) => Err(e),
                Err(_) => Err(PrivchatError::Generic {
                    msg: "authenticate: worker panic".into(),
                }),
            });
        });
        use std::sync::mpsc::RecvTimeoutError;
        rx.recv_timeout(std::time::Duration::from_secs(60))
            .map_err(|e| match e {
                RecvTimeoutError::Timeout => PrivchatError::Generic {
                    msg: "authenticate: timeout".into(),
                },
                RecvTimeoutError::Disconnected => PrivchatError::Generic {
                    msg: "authenticate: channel disconnected".into(),
                },
            })?
    }
    ...
    pub fn get_friends(
        ...
        limit: Option<u32>,
        offset: Option<u32>,
    ) -> Result<Vec<FriendEntry>, PrivchatError> {
        let inner = self.inner.clone();
        get_runtime().block_on(async move {
            let guard = inner.read().await;
            if let Some(sdk) = guard.as_ref() {
                ...
                let ffi_friends: Vec<FriendEntry> = pairs
                    ...
                    .map(|(f, u)| FriendEntry {
                        ...
                        nickname: u.nickname,
                        avatar_url: if u.avatar.is_empty() {
                            None
                        } else {
                            Some(u.avatar)
                        },
    ...
    pub fn sync_entities(
        ...
        use std::str::FromStr;
        let entity_type_parsed =
            privchat_sdk::EntityType::from_str(entity_type.as_str()).map_err(|_| {
                PrivchatError::Generic {
                    msg: format!("invalid entity_type: {}", entity_type),
                }
            })?;
        let inner = self.inner.clone();
        get_runtime().block_on(async move {
            ...
            if let Some(sdk) = guard.as_ref() {
                let count = sdk
                    .sync_entities(entity_type_parsed, scope.as_deref())
                    .await?;
                Ok(count as u32)
            } else {
                Err(PrivchatError::Disconnected)
    ...
    pub fn run_bootstrap_sync(&self) -> Result<(), PrivchatError> {
        ...
        let inner = self.inner.clone();
        std::thread::spawn(move || {
            let _ = tx.send(get_runtime().block_on(async move {
                let guard = inner.read().await;
                if let Some(sdk) = guard.as_ref() {
        ...
        rx.recv().map_err(|_| PrivchatError::Generic {
            msg: "run_bootstrap_sync: channel recv".into(),
        })?
    }

    /// 在后台执行启动同步（不阻塞）。用于已初始化时的增量补齐：is_bootstrap_completed 为 true 时调用。
    /// Runs in a dedicated Rust thread so we never block_on from the caller thread.
        ...
        get_runtime().block_on(async move {
            ...
            if let Some(sdk) = guard.as_ref() {
                ...
                serde_json::to_string(&map)
                    .map_err(|e| PrivchatError::Generic { msg: e.to_string() })
            } else {
                Err(PrivchatError::Disconnected)
            }
        })
                        ...
                        if let (Some(user_id), Some(username)) = (
                            ...
                            entries.push(UserEntry {
                                ...
                                user_type: user_value
                                    ...
                                    .and_then(|v| v.as_i64())
                                    .unwrap_or(0) as i16,
                                is_friend: user_value
                                    .get("is_friend")
                                    .and_then(|v| v.as_bool())
                                    .unwrap_or(false),
                                can_send_message: user_value
                                    .get("can_send_message")
                                    .and_then(|v| v.as_bool())
                                    .unwrap_or(false),
                                search_session_id: user_value
    ...
    pub fn send_friend_request(
        ...
        get_runtime().block_on(async move {
            ...
            if let Some(sdk) = guard.as_ref() {
                let result = sdk
                    .send_friend_request(to_user_id, remark.as_deref(), search_session_id)
                    .await?;
        ...
        get_runtime().block_on(async move {
            ...
            if let Some(sdk) = guard.as_ref() {
                ...
                let channel_id = result
                    .get("channel_id")
                    .and_then(|v| v.as_u64())
                    .ok_or_else(|| PrivchatError::Generic {
                        msg: "Missing channel_id in response".to_string(),
                    })?;
                Ok(channel_id)
            } else {
                Err(PrivchatError::Disconnected)
    ...
    pub fn create_group(
        ...
        name: String,
        member_ids: Vec<u64>,
    ) -> Result<GroupCreateResult, PrivchatError> {
        let inner = self.inner.clone();
        get_runtime().block_on(async move {
            let guard = inner.read().await;
            if let Some(sdk) = guard.as_ref() {
    ...
    pub fn get_message_history(
        ...
        get_runtime().block_on(async move {
            ...
            if let Some(sdk) = guard.as_ref() {
                ...
                let sdk_messages = sdk
                    .get_messages(channel_id, limit, before_message_id)
                    .await?;
    ...
    pub fn get_message_by_id(
        &self,
        message_id: u64,
    ) -> Result<Option<MessageEntry>, PrivchatError> {
    ...
    pub fn sync_messages(&self, channel_id: u64, limit: Option<u32>) -> Result<u32, PrivchatError> {
    ...
    pub fn subscribe_presence(
        &self,
        user_ids: Vec<u64>,
    ) -> Result<Vec<PresenceEntry>, PrivchatError> {
        let inner = self.inner.clone();
        get_runtime().block_on(async move {
            let guard = inner.read().await;
            if let Some(sdk) = guard.as_ref() {
                ...
                Ok(statuses
                    .into_iter()
                    .map(|(user_id, status)| {
                        Self::convert_online_status_to_presence_info(user_id, &status)
                    })
                    .collect())
            } else {
                Err(PrivchatError::Disconnected)
    ...
    pub fn on_app_foreground(self: Arc<Self>) -> Result<(), PrivchatError> {
        ...
        get_runtime().block_on(async move {
            let guard = this.inner.read().await;
            if let Some(sdk) = guard.as_ref() {
                sdk.on_app_foreground()
                    .await
                    .map_err(|e| PrivchatError::from(e))?;
                // 前台时后台同步好友列表（local-first：创建群组等可立即用最新好友）
                privchat_sdk::PrivchatSDK::sync_entities_in_background(
                    sdk.clone(),
                    privchat_sdk::EntityType::Friend,
                    None,
                );
                Ok(())
            } else {
    ...
    /// # 返回
    /// - `Ok(())`: 操作成功
    pub fn leave_channel(&self, channel_id: u64) -> Result<(), PrivchatError> {
        let inner = self.inner.clone();
        get_runtime().block_on(async move {
            let guard = inner.read().await;
            if let Some(sdk) = guard.as_ref() {
    ...
    pub fn list_members(
        ...
        get_runtime().block_on(async move {
            ...
            if let Some(sdk) = guard.as_ref() {
                ...
                let ffi_members: Vec<GroupMemberEntry> = members
                    .iter()
                    .map(|m| GroupMemberEntry {
                        user_id: m.member_uid,
                        channel_id: group_id,
    ...
    pub fn invite_user(
        ...
        get_runtime().block_on(async move {
            let guard = inner.read().await;
            if let Some(sdk) = guard.as_ref() {
                sdk.add_channel_members(channel_id, channel_type, user_ids)
                    .await?;
                Ok(())
            } else {
                Err(PrivchatError::Disconnected)
    ...
    pub fn kick_user(
        ...
        get_runtime().block_on(async move {
            let guard = inner.read().await;
            if let Some(sdk) = guard.as_ref() {
                sdk.remove_channel_member(channel_id, channel_type, user_id)
                    .await?;
                Ok(())
            } else {
                Err(PrivchatError::Disconnected)
    ...
    pub fn channel_profile(&self, channel_id: u64) -> Result<ChannelListEntry, PrivchatError> {
        let inner = self.inner.clone();
        get_runtime().block_on(async move {
            let guard = inner.read().await;
            if let Some(sdk) = guard.as_ref() {
                ...
                let entry = entries.first().ok_or_else(|| PrivchatError::Generic {
                    msg: "会话不存在".to_string(),
                })?;
    ...
    pub fn get_direct_channel_id_by_peer_user_id(
        &self,
        peer_user_id: u64,
    ) -> Result<Option<u64>, PrivchatError> {
        ...
        get_runtime().block_on(async move {
            ...
            if let Some(sdk) = guard.as_ref() {
                sdk.storage()
                    .find_channel_id_by_user(peer_user_id)
                    .await
    ...
    pub fn get_or_create_direct_channel(
        ...
        get_runtime().block_on(async move {
            let guard = inner.read().await;
            if let Some(sdk) = guard.as_ref() {
                let (channel_id, created) = sdk
                    .get_or_create_direct_channel(peer_user_id, source, source_id)
                    .await
                    .map_err(PrivchatError::from)?;
                Ok(GetOrCreateDirectChannelResult {
                    channel_id,
                    created,
                })
            } else {
    ...
    pub fn sync_channel(
        ...
        channel_id: u64,
        channel_type: u8,
    ) -> Result<SyncStateEntry, PrivchatError> {
        let inner = self.inner.clone();
        get_runtime().block_on(async move {
            let guard = inner.read().await;
            if let Some(sdk) = guard.as_ref() {
                ...
                let needs_sync = matches!(
                    sync_state.state,
                    SyncState::Syncing | SyncState::HasGap { .. }
                );
                Ok(SyncStateEntry {
        ...
        get_runtime().block_on(async move {
            ...
            if let Some(sdk) = guard.as_ref() {
                ...
                Ok(sync_states
                    ...
                    .map(|state| {
                        let needs_sync =
                            matches!(state.state, SyncState::Syncing | SyncState::HasGap { .. });
                        SyncStateEntry {
    ...
    pub fn get_channel_sync_state(
        ...
        get_runtime().block_on(async move {
            let guard = inner.read().await;
            if let Some(sdk) = guard.as_ref() {
                let (local_pts, server_pts) =
                    sdk.get_channel_sync_state(channel_id, channel_type).await?;

                // 检查是否需要同步
                let needs_sync = sdk
                    .needs_sync(channel_id, channel_type)
                    .await
                    .unwrap_or(false);
    ...
    pub fn start_supervised_sync(
        ...
        get_runtime().block_on(async move {
            ...
            if let Some(sdk) = guard.as_ref() {
                ...
                let callback: Arc<dyn Fn(privchat_sdk::events::SyncStatus) + Send + Sync> =
                    Arc::new(move |sdk_status| {
    ...
    pub fn search_channel(
        ...
        get_runtime().block_on(async move {
            ...
            if let Some(sdk) = guard.as_ref() {
                let page = sdk
                    .search_channel(channel_id, &query, limit, offset)
                    .await?;
                Ok(SearchPage {
                    hits: page
                        .hits
    ...
    pub fn channel_send_queue_set_enabled(
        ...
        get_runtime().block_on(async move {
            let guard = inner.read().await;
            if let Some(sdk) = guard.as_ref() {
                sdk.channel_send_queue_set_enabled(channel_id, enabled)
                    .await?;
                Ok(())
            } else {
                Err(PrivchatError::Disconnected)
    ...
    pub fn logout(&self) -> Result<(), PrivchatError> {
        ...
        rx.recv_timeout(std::time::Duration::from_secs(60))
            .map_err(|e| match e {
                RecvTimeoutError::Timeout => PrivchatError::Generic {
                    msg: "logout: timeout".into(),
                },
                RecvTimeoutError::Disconnected => PrivchatError::Generic {
                    msg: "logout: channel disconnected".into(),
                },
            })?
    }
    ...
    pub fn set_channel_low_priority(
        ...
        get_runtime().block_on(async move {
            let guard = inner.read().await;
            if let Some(sdk) = guard.as_ref() {
                sdk.set_channel_low_priority(channel_id, low_priority)
                    .await?;
                Ok(())
            } else {
                Err(PrivchatError::Disconnected)
    ...
    /// # 返回
    /// - `Ok(ChannelTags)`: 会话标签
    pub fn channel_tags(&self, channel_id: u64) -> Result<ChannelTags, PrivchatError> {
        let inner = self.inner.clone();
        get_runtime().block_on(async move {
            let guard = inner.read().await;
            if let Some(sdk) = guard.as_ref() {
    ...
    pub fn set_channel_notification_mode(
        ...
        get_runtime().block_on(async move {
            ...
            if let Some(sdk) = guard.as_ref() {
                ...
                sdk.set_channel_notification_mode(channel_id, sdk_mode)
                    .await?;
                Ok(())
            } else {
                Err(PrivchatError::Disconnected)
    ...
    pub fn dm_peer_user_id(&self, channel_id: u64) -> Result<Option<u64>, PrivchatError> {
        ...
        get_runtime().block_on(async move {
            let guard = inner.read().await;
            if let Some(sdk) = guard.as_ref() {
                sdk.dm_peer_user_id(channel_id)
                    .await
                    .map_err(|e| PrivchatError::from(e))
            } else {
                Err(PrivchatError::Disconnected)
    ...
    pub fn reactions(
        ...
        channel_id: u64,
        message_id: u64,
    ) -> Result<Vec<ReactionChip>, PrivchatError> {
        let inner = self.inner.clone();
        get_runtime().block_on(async move {
            let guard = inner.read().await;
            if let Some(sdk) = guard.as_ref() {
    ...
    pub fn is_event_read_by(
        ...
        get_runtime().block_on(async move {
            let guard = inner.read().await;
            if let Some(sdk) = guard.as_ref() {
                sdk.is_event_read_by(channel_id, message_id, user_id)
                    .await
                    .map_err(|e| PrivchatError::from(e))
            } else {
                Err(PrivchatError::Disconnected)
...
impl PrivchatSDK {
    ...
    fn convert_online_status_to_presence_info(
        ...
        // 获取设备类型（取第一个设备）
        let device_type = status.online_devices.first().map(|d| format!("{:?}", d));
                    ...
                    Ok(sdk_event) => {
                        ...
                        } = &sdk_event
                        {
                            ...
                            let send_state = match state {
                                privchat_sdk::events::SendStatusState::Enqueued => {
                                    SendState::Enqueued
                                }
                                privchat_sdk::events::SendStatusState::Sending => {
                                    SendState::Sending
                                }
                                privchat_sdk::events::SendStatusState::Sent => SendState::Sent,
                                privchat_sdk::events::SendStatusState::Retrying => {
                                    SendState::Retrying
                                }
                        ...
                        } = &sdk_event
                        {
                            ...
                            let ffi_diff = match diff_kind {
                                SdkDiffKind::Reset { messages } => TimelineDiffKind::Reset {
                                    values: messages
                                        .iter()
                                        .map(|m| Self::convert_timeline_message(m))
                                        .collect(),
                                },
                                SdkDiffKind::Append { messages } => TimelineDiffKind::Append {
                                    values: messages
                                        .iter()
                                        .map(|m| Self::convert_timeline_message(m))
                                        .collect(),
                                ...
                                SdkDiffKind::RemoveByItemId { item_id } => {
                                    TimelineDiffKind::RemoveByItemId { item_id: *item_id }
                                }
                            };
                        ...
                        if let privchat_sdk::events::SDKEvent::TypingIndicator(typing_event) =
                            &sdk_event
                        {
                        ...
                        if let privchat_sdk::events::SDKEvent::ReadReceiptReceived(receipt_event) =
                            &sdk_event
                        {
                        ...
                        } = &sdk_event
                        {
                            ...
                            match update_kind {
                                SdkUpdateKind::Reset { channels } => {
                                    let ffi_entries: Vec<ChannelListEntry> = channels
                                        .iter()
                                ...
                                SdkUpdateKind::Update { channel } => {
                                    let ffi_entry =
                                        Self::convert_sdk_channel_list_entry_to_ffi(channel);
                                    let guard = channel_list_observers.lock().await;
                                    for observer in guard.values() {
    ...
    fn convert_sdk_event(sdk_event: &privchat_sdk::events::SDKEvent) -> Option<SDKEvent> {
        ...
        let event_type = match sdk_event {
            ...
            SdkEvent::TypingIndicator(_)
            | SdkEvent::TypingStarted(_)
            | SdkEvent::TypingStopped(_) => EventType::TypingIndicator,
    ...
    pub fn observe_sends(self: Arc<Self>, observer: Box<dyn SendObserver>) -> u64 {
    ...
    pub fn unobserve_sends(self: Arc<Self>, token: u64) -> bool {
        let observers = self.send_observers.clone();
        let removed =
            get_runtime().block_on(async move { observers.lock().await.remove(&token).is_some() });
    ...
    pub fn observe_timeline(
        ...
        info!(
            "Timeline observer registered with token: {} for channel: {}",
            token, channel_id
        );
        token
    }
    ...
    pub fn unobserve_timeline(self: Arc<Self>, token: u64) -> bool {
        let observers = self.timeline_observers.clone();
        let removed =
            get_runtime().block_on(async move { observers.lock().await.remove(&token).is_some() });
    ...
    pub fn observe_channel_list(self: Arc<Self>, observer: Box<dyn ChannelListObserver>) -> u64 {
        let token = self
            .channel_list_observer_counter
            .fetch_add(1, Ordering::Relaxed)
            + 1;
    ...
    pub fn unobserve_channel_list(self: Arc<Self>, token: u64) -> bool {
        let observers = self.channel_list_observers.clone();
        let removed =
            get_runtime().block_on(async move { observers.lock().await.remove(&token).is_some() });
    ...
    pub fn observe_typing(
        ...
        info!(
            "Typing observer registered with token: {} for channel: {}",
            token, channel_id
        );
        token
    }
    ...
    pub fn unobserve_typing(self: Arc<Self>, token: u64) -> bool {
        let observers = self.typing_observers.clone();
        let removed =
            get_runtime().block_on(async move { observers.lock().await.remove(&token).is_some() });
    ...
    pub fn observe_receipts(
        ...
        info!(
            "Receipts observer registered with token: {} for channel: {}",
            token, channel_id
        );
        token
    }
    ...
    pub fn unobserve_receipts(self: Arc<Self>, token: u64) -> bool {
        let observers = self.receipts_observers.clone();
        let removed =
            get_runtime().block_on(async move { observers.lock().await.remove(&token).is_some() });
    ...
    fn convert_sdk_message_to_ffi(
        ...
        let timestamp_ms = sdk_msg.timestamp.filter(|&ts| ts > 0).unwrap_or_else(|| {
...
/// Login by registry id (from register_sdk_handle). No pointer/from_raw; avoids dealloc crash on iOS.
#[uniffi::export]
pub fn login_by_handle(
    id: u64,
    username: String,
    password: String,
    device_id: String,
    ...
    eprintln!(
        "[FFI] login_by_handle entry id={} username={}",
        id, username
    );
    info!(
        "[FFI] login_by_handle entry id={} username={}",
    ...
    let arc = match sdk_registry().lock() {
        Ok(reg) => reg.get(&id).cloned(),
        Err(_) => {
            error!("[FFI] login_by_handle registry lock failed");
            return Err(PrivchatError::Generic {
                msg: "registry lock".into(),
            });
        }
    };
    let arc = match arc {
        Some(a) => {
            eprintln!(
                "[FFI] login_by_handle registry ok id={}, spawning worker",
                id
            );
            info!(
                "[FFI] login_by_handle registry ok id={}, spawning worker",
        ...
        None => {
            eprintln!("[FFI] login_by_handle SDK not in registry id={}", id);
            warn!("[FFI] login_by_handle SDK not in registry id={}", id);
            return Err(PrivchatError::Generic { msg: format!("login_by_handle: SDK not in registry (id={}), ensure Rust was built with SDK_NEXT_ID=0", id) });
        }
    };
    let (tx, rx) = std::sync::mpsc::sync_channel(0);
    let inner = arc.inner.clone();
    std::thread::spawn(move || {
        #[cfg(target_os = "ios")]
        unsafe {
            extern "C" {
        ...
        eprintln!("[FFI] login_by_handle worker thread started");
        info!("[FFI] login_by_handle worker thread started");
        let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
            let rt = tokio::runtime::Builder::new_current_thread()
                .enable_all()
                .build()
                .map_err(|e| PrivchatError::Generic {
                    msg: format!("login runtime: {}", e),
                })?;
            rt.block_on(async move {
                eprintln!("[FFI] login_by_handle block_on entered, reading inner");
                info!("[FFI] login_by_handle block_on entered, reading inner");
                let guard = inner.read().await;
                let has_sdk = guard.as_ref().is_some();
                eprintln!("[FFI] login_by_handle inner.has_sdk={}", has_sdk);
                info!("[FFI] login_by_handle inner.has_sdk={}", has_sdk);
                if let Some(sdk) = guard.as_ref() {
                    eprintln!("[FFI] login_by_handle calling sdk.login() (sending RPC)");
                    info!("[FFI] login_by_handle calling sdk.login() (sending RPC)");
                    let (user_id, token) = sdk.login(username, password, device_id, None).await?;
                    eprintln!("[FFI] login_by_handle sdk.login() ok user_id={}", user_id);
                    info!("[FFI] login_by_handle sdk.login() ok user_id={}", user_id);
                    Ok(AuthResult { user_id, token })
                } else {
                    eprintln!("[FFI] login_by_handle SDK inner is None");
                    warn!("[FFI] login_by_handle SDK inner is None");
                    Err(PrivchatError::Generic { msg: "login_by_handle: SDK inner not initialized (connect may not have completed)".into() })
                }
            })
        }));
        let to_send: Result<AuthResult, PrivchatError> = match result {
            Ok(Ok(v)) => {
                eprintln!(
                    "[FFI] login_by_handle worker result Ok(Ok) user_id={}",
                    v.user_id
                );
                info!(
                    "[FFI] login_by_handle worker result Ok(Ok) user_id={}",
            ...
            Ok(Err(e)) => {
                eprintln!(
                    "[FFI] login_by_handle worker result Ok(Err): {}",
                    e.message()
                );
                warn!(
                    "[FFI] login_by_handle worker result Ok(Err): {}",
            ...
            Err(_) => {
                eprintln!("[FFI] login_by_handle worker panic");
                error!("[FFI] login_by_handle worker panic");
                Err(PrivchatError::Generic {
                    msg: "login_by_handle: panic".into(),
                })
            }
        };
    ...
    use std::sync::mpsc::RecvTimeoutError;
    eprintln!("[FFI] login_by_handle waiting recv_timeout 60s");
    info!("[FFI] login_by_handle waiting recv_timeout 60s");
    let out = rx
        .recv_timeout(std::time::Duration::from_secs(60))
        .map_err(|e| {
            eprintln!("[FFI] login_by_handle recv_timeout failed: {:?}", e);
            warn!("[FFI] login_by_handle recv_timeout failed: {:?}", e);
            match e {
                RecvTimeoutError::Timeout => PrivchatError::Generic {
                    msg: "login: timeout".into(),
                },
                RecvTimeoutError::Disconnected => PrivchatError::Generic {
                    msg: "login: disconnected".into(),
                },
            }
        })??;
    eprintln!(
        "[FFI] login_by_handle returning Ok(user_id={})",
        out.user_id
    );
    info!(
        "[FFI] login_by_handle returning Ok(user_id={})",
