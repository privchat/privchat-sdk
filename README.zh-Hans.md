...
- **数据同步**：Bootstrap 全量同步、重连恢复同步、增量差分同步、按实体类型 / 频道粒度同步
...
| `resume sync` | 断线重连后基于 `since_version` 和频道 `pts` 的恢复同步 |
...
- `privchat-server` 全量 `cargo test`
- 本地数据库集成测试 `entity_sync_version_db_test`
  （当设置 `DATABASE_URL` 或 `PRIVCHAT_TEST_DATABASE_URL` 时真正连接数据库运行）
- `privchat-sdk` 关键恢复与实体版本测试
- `accounts` example smoke 流程
...
- [同步恢复规范](../privchat-docs/spec/03-protocol-sdk/SDK_SYNC_RESUME_SPEC.md) — 基于实体版本和频道 `pts` 的重连恢复同步
